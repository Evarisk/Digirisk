 ...
 */
class InformationsReleaseIk extends CoreObject
{
	/**
	 ...
	 */
	public $table_element = 'informationsrelease_ik';
	...
	public static function getRangesByCategory($fk_c_exp_tax_cat, $active = 1)
	{
		...
		if ($resql)
		{
			...
			if ($num > 0)
			{
				while ($obj = $db->fetch_object($resql))
				{
					$object = new InformationsReleaseIk($db);
	...
	public static function getAllRanges()
	{
		...
		$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'informationsrelease_ik i ON (r.rowid = i.fk_range)';
		...
		if ($resql)
		{
			while ($obj = $db->fetch_object($resql))
			{
				$ik = new InformationsReleaseIk($db);
