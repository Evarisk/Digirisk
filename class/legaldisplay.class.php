...
class LegalDisplay extends CommonObject
{
	 ...
	 */
	public $table_element = 'digiriskdolibarr_legaldisplay';
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->digiriskdolibarr->legaldisplay->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->digiriskdolibarr->legaldisplay->legaldisplay_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			{
				// Call trigger
				$result = $this->call_trigger('LEGALDISPLAY_VALIDATE', $user);
				if ($result < 0) $error++;
				// End call triggers
			}
		}
		...
		if (!$error)
		{
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref))
			{
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'legaldisplay/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'legaldisplay/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				...
				$dirsource = $conf->digiriskdolibarr->dir_output.'/legaldisplay/'.$oldref;
				$dirdest = $conf->digiriskdolibarr->dir_output.'/legaldisplay/'.$newref;
				if (!$error && file_exists($dirsource))
				{
					...
					if (@rename($dirsource, $dirdest))
					{
						...
						$listoffiles = dol_dir_list($conf->digiriskdolibarr->dir_output.'/legaldisplay/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'LEGALDISPLAY_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'LEGALDISPLAY_CLOSE');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'LEGALDISPLAY_REOPEN');
	}
	...
	public function getLinesArray()
	{
		...
		$objectline = new LegalDisplayLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_legaldisplay = '.$this->id));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->DIGIRISKDOLIBARR_LEGALDISPLAY_ADDON)) {
			$conf->global->DIGIRISKDOLIBARR_LEGALDISPLAY_ADDON = 'mod_legaldisplay_standard';
		}
		...
		if (!empty($conf->global->DIGIRISKDOLIBARR_LEGALDISPLAY_ADDON))
		{
			...
			$file = $conf->global->DIGIRISKDOLIBARR_LEGALDISPLAY_ADDON.".php";
			$classname = $conf->global->DIGIRISKDOLIBARR_LEGALDISPLAY_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_legaldisplay';

			if ($this->modelpdf) {
				$modele = $this->modelpdf;
			} elseif (!empty($conf->global->LEGALDISPLAY_ADDON_PDF)) {
				$modele = $conf->global->LEGALDISPLAY_ADDON_PDF;
			}
		}
