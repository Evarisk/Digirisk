...
/**
 *       \file       htdocs/informationsrelease/class/informationsrelease.class.php
 *       \ingroup    informationsrelease
 *       \brief      File to manage Expense Reports
 */
require_once DOL_DOCUMENT_ROOT.'/core/class/commonobject.class.php';
require_once DOL_DOCUMENT_ROOT.'/custom/DigiriskDolibarr/class/informationsrelease_ik.class.php';
require_once DOL_DOCUMENT_ROOT.'/custom/DigiriskDolibarr/class/informationsrelease_rule.class.php';

/**
 * Class to manage Trips and Expenses
 */
class InformationsRelease extends CommonObject
{
    /**
	 * @var string ID to identify managed object
	 */
	public $element = 'informationsrelease';

    /**
	 * @var string Name of table without prefix where object is stored
	 */
	public $table_element = 'informationsrelease';

    public $table_element_line = 'informationsrelease_det';
    public $fk_element = 'fk_informationsrelease';
            ...
            if (!$error)
            {
                if (is_array($this->lines) && count($this->lines) > 0)
                {
    	            foreach ($this->lines as $line)
    	            {
    	            	...
    	            	if (!is_object($line)) {
    	            		$line = (object) $line;
    	            		$newndfline = new InformationsReleaseLine($this->db);
    	            		$newndfline->fk_informationsrelease = $line->fk_informationsrelease;
    	                ...
    	                //$newndfline=new InformationsReleaseLine($this->db);
    	                $newndfline->fk_informationsrelease = $this->id;
	 ...
	 * @param   int     $notrigger              Disable triggers
     * @param   User    $userofinformationsrelease    New user we want to have the expense report on.
     * @return  int                             <0 if KO, >0 if OK
     */
    public function update($user, $notrigger = 0, $userofinformationsrelease = null)
    {
        global $langs;
        ...
        if ($userofinformationsrelease && is_object($userofinformationsrelease))
        {
            $sql .= " , fk_user_author = ".($userofinformationsrelease->id > 0 ? "'".$userofinformationsrelease->id."'" : "null"); // Note fk_user_author is not the 'author' but the guy the expense report is for.
        }
    ...
    public function set_paid($id, $fuser, $notrigger = 0)
    {
        ...
        $sql = "UPDATE ".MAIN_DB_PREFIX."informationsrelease";
    ...
    public function info($id)
    {
        ...
        $sql .= " FROM ".MAIN_DB_PREFIX."informationsrelease as f";
    ...
    public function initAsSpecimen()
    {
        ...
        $xnbp = 0;
        while ($xnbp < $nbp) {
            $line = new InformationsReleaseLine($this->db);
    ...
    public function fetch_line_by_project($projectid, $user = '')
    {
        ...
        if ($user->rights->informationsrelease->lire) {
            $sql = "SELECT de.fk_informationsrelease, de.date, de.comments, de.total_ht, de.total_ttc";
            $sql .= " FROM ".MAIN_DB_PREFIX."informationsrelease_det as de";
            ...
            if ($result)
            {
                ...
                while ($i < $num)
                {
                    ...
                    $sql2 .= " FROM ".MAIN_DB_PREFIX."informationsrelease as d";
                    $sql2 .= " WHERE d.rowid = '".$objp->fk_informationsrelease."'";
                    ...
                    $objp->ref = $obj->ref;
                    $objp->fk_c_informationsrelease_status = $obj->fk_statut;
                    ...
                    print '<tr>';
                    print '<td><a href="'.DOL_URL_ROOT.'/informationsrelease/card.php?id='.$objp->rowid.'">'.$objp->ref_num.'</a></td>';
                    ...
                    switch ($objp->fk_c_informationsrelease_status) {
                        case 4:
    ...
    public function fetch_lines()
    {
        ...
        if (!empty($conf->global->INFORMATIONSRELEASE_LINES_SORTED_BY_ROWID))
        {
        ...
        if ($resql)
        {
            ...
            while ($i < $num)
            {
                ...
                $deplig = new InformationsReleaseLine($this->db);
                ...
                $deplig->fk_informationsrelease = $objp->fk_informationsrelease;
        ...
        if ($resql)
        {
			...
			if (!$error)
			{
			    ...
			    if (preg_match('/^[\(]?PROV/i', $this->ref))
			    {
			    	...
			    	$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'informationsrelease/".$this->db->escape($this->newref)."'";
			    	$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'informationsrelease/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
					$resql = $this->db->query($sql);
					...
					$dirsource = $conf->informationsrelease->dir_output.'/'.$oldref;
					$dirdest = $conf->informationsrelease->dir_output.'/'.$newref;
					if (!$error && file_exists($dirsource))
					{
					    ...
					    if (@rename($dirsource, $dirdest))
					    {
					        ...
					        $listoffiles = dol_dir_list($conf->informationsrelease->dir_output.'/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
    ...
    public function set_save_from_refuse($fuser)
    {
        ...
        {
            dol_syslog(get_class($this)."::set_save_from_refuse informationsrelease already with save status", LOG_WARNING);
        }
    }
    ...
    public function setApproved($fuser, $notrigger = 0)
    {
        ...
        {
            dol_syslog(get_class($this)."::setApproved informationsrelease already with approve status", LOG_WARNING);
        }
    ...
    public function setDeny($fuser, $details, $notrigger = 0)
    {
        ...
        {
            dol_syslog(get_class($this)."::setDeny informationsrelease already with refuse status", LOG_WARNING);
        }
    }
    ...
    public function set_unpaid($fuser, $notrigger = 0)
    {
        ...
        {
            dol_syslog(get_class($this)."::set_unpaid informationsrelease already with unpaid status", LOG_WARNING);
        }
    }
    ...
    public function set_cancel($fuser, $detail, $notrigger = 0)
    {
        ...
        {
            dol_syslog(get_class($this)."::set_cancel informationsrelease already with cancel status", LOG_WARNING);
        }
    }
    ...
    public function getNextNumRef()
    {
        ...
        if (!empty($conf->global->INFORMATIONSRELEASE_ADDON))
        {
        	$mybool = false;

        	$file = $conf->global->INFORMATIONSRELEASE_ADDON.".php";
			$classname = $conf->global->INFORMATIONSRELEASE_ADDON;

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir)
			{
                $dir = dol_buildpath($reldir."core/modules/informationsrelease/");
        ...
        {
            $this->error = "Error_INFORMATIONSRELEASE_ADDON_NotDefined";
            return -2;
        }
    }
    ...
    public function getNomUrl($withpicto = 0, $max = 0, $short = 0, $moretitle = '', $notooltip = 0, $save_lastsearch_value = -1)
    {
        ...
        $url = DOL_URL_ROOT.'/informationsrelease/card.php?id='.$this->id;

        if ($short) return $url;

        $label = '<u>'.$langs->trans("ShowInformationsRelease").'</u>';
        if (!empty($this->ref))
        ...
        if (empty($notooltip))
        {
            ...
            {
                $label = $langs->trans("ShowInformationsRelease");
	 ...
	 * @param    int         $fk_ecm_files             Id of ECM file to link to this informationsrelease line
	 * @return   int                                   <0 if KO, >0 if OK
	 */
    public function addline($qty = 0, $up = 0, $fk_c_type_fees = 0, $vatrate = 0, $date = '', $comments = '', $fk_project = 0, $fk_c_exp_tax_cat = 0, $type = 0, $fk_ecm_files = 0)
	{
		...
		if ($this->fk_statut == self::STATUS_DRAFT)
		{
			...
			$this->line = new InformationsReleaseLine($this->db);
			...
			$this->line->fk_informationsrelease = $this->id;
			$this->line->qty = $qty;
			$this->line->date = $date;
	...
	public function checkRules($type = 0, $seller = '')
	{
		...
		$rulestocheck = InformationsReleaseRule::getAllRule($this->line->fk_c_type_fees, $this->line->date, $this->fk_user_author);
		...
		foreach ($rulestocheck as $rule)
		{
			...
			if ($amount_to_test > $rule->amount)
			{
				...
				if ($rule->restrictive)
				{
					$this->error = 'InformationsReleaseConstraintViolationError';
					$this->errors[] = $this->error;

					$new_current_total_ttc -= $amount_to_test - $rule->amount; // ex, entered 16€, limit 12€, subtracts 4€;
					$rule_warning_message_tab[] = $langs->trans('InformationsReleaseConstraintViolationError', $rule->id, price($amount_to_test, 0, $langs, 1, -1, -1, $conf->currency), price($rule->amount, 0, $langs, 1, -1, -1, $conf->currency), $langs->trans('by'.$rule->code_expense_rules_type, price($new_current_total_ttc, 0, $langs, 1, -1, -1, $conf->currency)));
				}
				else
				{
					$this->error = 'InformationsReleaseConstraintViolationWarning';
					$this->errors[] = $this->error;

					$rule_warning_message_tab[] = $langs->trans('InformationsReleaseConstraintViolationWarning', $rule->id, price($amount_to_test, 0, $langs, 1, -1, -1, $conf->currency), price($rule->amount, 0, $langs, 1, -1, -1, $conf->currency), $langs->trans('nolimitby'.$rule->code_expense_rules_type));
				}
	...
	public function applyOffset()
	{
		...
		$range = InformationsReleaseIk::getRangeByUser($userauthor, $this->line->fk_c_exp_tax_cat);
	...
	{
		$sql = 'SELECT e.rowid FROM '.MAIN_DB_PREFIX.'informationsrelease e';
		$sql .= ' INNER JOIN '.MAIN_DB_PREFIX.'informationsrelease_det d ON (e.rowid = d.fk_informationsrelease)';
     ...
     * @param   int         $date                   Date
     * @param   int         $informationsrelease_id       Expense report id
     * @param   int         $fk_c_exp_tax_cat       Id of category of car
	 * @param   int         $fk_ecm_files           Id of ECM file to link to this informationsrelease line
     * @return  int                                 <0 if KO, >0 if OK
     */
    public function updateline($rowid, $type_fees_id, $projet_id, $vatrate, $comments, $qty, $value_unit, $date, $informationsrelease_id, $fk_c_exp_tax_cat = 0, $fk_ecm_files = 0)
    {
        global $user, $mysoc;
        ...
        if ($this->fk_statut == 0 || $this->fk_statut == 99)
        {
            ...
            $this->line->fk_informationsrelease = $informationsrelease_id;
     ...
     */
    public function fetch_users_approver_informationsrelease()
    {
        // phpcs:enable
        ...
        $sql .= " WHERE ur.fk_id = rd.id and rd.module = 'informationsrelease' AND rd.perms = 'approve'"; // Permission 'Approve';
        $sql .= "UNION";
        $sql .= " SELECT DISTINCT ugu.fk_user";
        $sql .= " FROM ".MAIN_DB_PREFIX."usergroup_user as ugu, ".MAIN_DB_PREFIX."usergroup_rights as ur, ".MAIN_DB_PREFIX."rights_def as rd";
        $sql .= " WHERE ugu.fk_usergroup = ur.fk_usergroup AND ur.fk_id = rd.id and rd.module = 'informationsrelease' AND rd.perms = 'approve'"; // Permission 'Approve';
        //print $sql;

        dol_syslog(get_class($this)."::fetch_users_approver_informationsrelease sql=".$sql);
        $result = $this->db->query($sql);
        if ($result)
        {
            $num_rows = $this->db->num_rows($result); $i = 0;
        ...
        {
            $this->error = $this->db->lasterror();
            dol_syslog(get_class($this)."::fetch_users_approver_informationsrelease  Error ".$this->error, LOG_ERR);
            return -1;
        }
    }
    ...
    public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
    {
	    ...
	    if (!dol_strlen($modele)) {
		    ...
		    } elseif (!empty($conf->global->INFORMATIONSRELEASE_ADDON_PDF)) {
			    $modele = $conf->global->INFORMATIONSRELEASE_ADDON_PDF;
		    }
	    }

        $modelpath = "core/modules/informationsrelease/doc/";
    ...
    public function load_state_board()
    {
        ...
        $sql .= " FROM ".MAIN_DB_PREFIX."informationsrelease as ex";
        $sql .= " WHERE ex.fk_statut > 0";
        $sql .= " AND ex.entity IN (".getEntity('informationsrelease').")";
		if (empty($user->rights->informationsrelease->readall))
		{
        ...
        if ($resql) {
            while ($obj = $this->db->fetch_object($resql)) {
                $this->nb["informationsreleases"] = $obj->nb;
            }
            $this->db->free($resql);
            return 1;
        }
    ...
    public function load_board($user, $option = 'topay')
    {
        ...
        if ($resql)
        {
	        ...
	        if ($option == 'toapprove')
	        {
	            $response->warning_delay = $conf->informationsrelease->approve->warning_delay / 60 / 60 / 24;
	            $response->label = $langs->trans("InformationsReleasesToApprove");
	            $response->labelShort = $langs->trans("ToApprove");
	            $response->url = DOL_URL_ROOT.'/informationsrelease/list.php?mainmenu=hrm&amp;statut=2';
	        }
	        else
	        {
	            $response->warning_delay = $conf->informationsrelease->payment->warning_delay / 60 / 60 / 24;
	            $response->label = $langs->trans("InformationsReleasesToPay");
	            $response->labelShort = $langs->trans("StatusToPay");
	            $response->url = DOL_URL_ROOT.'/informationsrelease/list.php?mainmenu=hrm&amp;statut=5';
	        }
            ...
            while ($obj = $this->db->fetch_object($resql))
            {
	            ...
	            if ($option == 'toapprove')
	            {
	                if ($this->db->jdate($obj->date_valid) < ($now - $conf->informationsrelease->approve->warning_delay)) {
	                    $response->nbtodolate++;
	                }
	            }
	            else
	            {
                    if ($this->db->jdate($obj->date_valid) < ($now - $conf->informationsrelease->payment->warning_delay)) {
    	                $response->nbtodolate++;
                    }
	            }
            }
    ...
    public function hasDelay($option)
    {
        ...
        if ($option == 'toapprove')
        {
            return ($this->datevalid ? $this->datevalid : $this->date_valid) < ($now - $conf->informationsrelease->approve->warning_delay);
        }
        else
            return ($this->datevalid ? $this->datevalid : $this->date_valid) < ($now - $conf->informationsrelease->payment->warning_delay);
    }

    /**
     *	Return if an informationsrelease was dispatched into bookkeeping
     *
	...
	{
		$table = 'payment_informationsrelease';
		$field = 'fk_informationsrelease';
...
class InformationsReleaseLine
{
    ...
    {
        $sql = 'SELECT fde.rowid, fde.fk_informationsrelease, fde.fk_c_type_fees, fde.fk_c_exp_tax_cat, fde.fk_projet as fk_project, fde.date,';
        ...
        $sql .= ' FROM '.MAIN_DB_PREFIX.'informationsrelease_det as fde';
        ...
        if ($result)
        {
            ...
            $this->id = $objp->rowid;
            $this->ref = $objp->ref;
            $this->fk_informationsrelease = $objp->fk_informationsrelease;
    ...
    public function insert($notrigger = 0, $fromaddline = false)
    {
        ...
        dol_syslog("InformationsReleaseLine::Insert rang=".$this->rang, LOG_DEBUG);
        ...
        $sql = 'INSERT INTO '.MAIN_DB_PREFIX.'informationsrelease_det';
        $sql .= ' (fk_informationsrelease, fk_c_type_fees, fk_projet,';
        ...
        $sql .= " VALUES (".$this->db->escape($this->fk_informationsrelease).",";
        ...
        if ($resql)
        {
            $this->id = $this->db->last_insert_id(MAIN_DB_PREFIX.'informationsrelease_det');

			if (!$fromaddline)
			{
				$tmpparent = new InformationsRelease($this->db);
				$tmpparent->fetch($this->fk_informationsrelease);
        ...
        {
            $this->error = $this->db->lasterror();
            dol_syslog("InformationsReleaseLine::insert Error ".$this->error, LOG_ERR);
            $this->db->rollback();
            return -2;
        }
    }
	 ...
	 *
	 * @param  InformationsReleaseRule $rule		object rule to check
	 * @param  int				 $fk_user	user author id
	 * @param  string			 $mode		day|EX_DAY / month|EX_MON / year|EX_YEA to get amount
	 * @return float                        Amount
	 */
	public function getExpAmount(InformationsReleaseRule $rule, $fk_user, $mode = 'day')
	{
		$amount = 0;

		$sql = 'SELECT SUM(d.total_ttc) as total_amount';
		$sql .= ' FROM '.MAIN_DB_PREFIX.'informationsrelease_det d';
		$sql .= ' INNER JOIN '.MAIN_DB_PREFIX.'informationsrelease e ON (d.fk_informationsrelease = e.rowid)';
		...
		dol_syslog('InformationsReleaseLine::getExpAmount');
    ...
    public function update(User $user)
    {
        ...
        $sql = "UPDATE ".MAIN_DB_PREFIX."informationsrelease_det SET";
        ...
        dol_syslog("InformationsReleaseLine::update sql=".$sql);

        $resql = $this->db->query($sql);
        if ($resql)
        {
            $tmpparent = new InformationsRelease($this->db);
            $result = $tmpparent->fetch($this->fk_informationsrelease);
            if ($result > 0)
            {
                $result = $tmpparent->update_price();
                if ($result < 0)
        ...
        {
            $this->error = $this->db->lasterror();
            dol_syslog("InformationsReleaseLine::update Error ".$this->error, LOG_ERR);
            $this->db->rollback();
            return -2;
        }
    }
 ...
 *    Les valeurs de la liste sont les id de la table c_informationsrelease_statuts
 *
 ...
 */
function select_informationsrelease_statut($selected = '', $htmlname = 'fk_statut', $useempty = 1, $useshortlabel = 0)
{
    global $db, $langs;

    $tmpep = new InformationsRelease($db);
