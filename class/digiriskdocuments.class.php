...
class DigiriskDocuments extends CommonObject
{
	 ...
	 */
	//public $table_element_line = 'digiriskdolibarr_digiriskdocumentsline';

	/**
	 * @var int    Field with ID of parent key if this object has a parent
	 */
	//public $fk_element = 'fk_digiriskdocuments';

	/**
	 * @var int    Name of subtable class that manage subtable lines
	 */
	//public $class_element_line = 'DigiriskDocumentsline';
	 ...
	 */
	//protected $childtablesoncascade = array('digiriskdolibarr_digiriskdocumentsdet');

	/**
	 * @var DigiriskDocumentsLine[]     Array of subtable lines
	 */
	//public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->digiriskdolibarr->digiriskdocuments->read) {
	...
	public function DigiriskFillJSON($object) {
		...
		switch ($object->element) {
			...
			case "firepermit":
				...
				break;
			case "preventionplan":
				$this->json = $this->PreventionPlanFillJSON($object);
				break;
		}
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->digiriskdolibarr->digiriskdocuments->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->digiriskdolibarr->digiriskdocuments->digiriskdocuments_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			{
				// Call trigger
				$result = $this->call_trigger('DIGIRISKDOCUMENTS_VALIDATE', $user);
				if ($result < 0) $error++;
				// End call triggers
			}
		}
		...
		if (!$error)
		{
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref))
			{
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'digiriskdocuments/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'digiriskdocuments/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				...
				$dirsource = $conf->digiriskdolibarr->dir_output.'/digiriskdocuments/'.$oldref;
				$dirdest = $conf->digiriskdolibarr->dir_output.'/digiriskdocuments/'.$newref;
				if (!$error && file_exists($dirsource))
				{
					...
					if (@rename($dirsource, $dirdest))
					{
						...
						$listoffiles = dol_dir_list($conf->digiriskdolibarr->dir_output.'/digiriskdocuments/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'DIGIRISKDOCUMENTS_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'DIGIRISKDOCUMENTS_CLOSE');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'DIGIRISKDOCUMENTS_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = '<u>'.$langs->trans("DigiriskDocuments").'</u>';
		$label .= '<br>';
		...
		$url = dol_buildpath('/digiriskdolibarr/digiriskdocuments_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip))
		{
			...
			{
				$label = $langs->trans("ShowDigiriskDocuments");
		...
		$hookmanager->initHooks(array('digiriskdocumentsdao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new DigiriskDocumentsLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_digiriskdocuments = '.$this->id));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->DIGIRISKDOLIBARR_DIGIRISKDOCUMENTS_ADDON)) {
			$conf->global->DIGIRISKDOLIBARR_DIGIRISKDOCUMENTS_ADDON = 'mod_digiriskdocuments_standard';
		}
		...
		if (!empty($conf->global->DIGIRISKDOLIBARR_DIGIRISKDOCUMENTS_ADDON))
		{
			...
			$file = $conf->global->DIGIRISKDOLIBARR_DIGIRISKDOCUMENTS_ADDON.".php";
			$classname = $conf->global->DIGIRISKDOLIBARR_DIGIRISKDOCUMENTS_ADDON;
