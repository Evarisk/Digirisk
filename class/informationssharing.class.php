 ...
 */
class InformationsSharing extends CommonObject
{
	/**
	 ...
	 */
	public $table_element = 'digiriskdolibarr_informationssharing';
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->digiriskdolibarr->informationssharing->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->digiriskdolibarr->informationssharing->informationssharing_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			{
				// Call trigger
				$result = $this->call_trigger('INFORMATIONSSHARING_VALIDATE', $user);
				if ($result < 0) $error++;
				// End call triggers
			}
		}
		...
		if (!$error)
		{
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref))
			{
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'informationssharing/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'informationssharing/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				...
				$dirsource = $conf->digiriskdolibarr->dir_output.'/informationssharing/'.$oldref;
				$dirdest = $conf->digiriskdolibarr->dir_output.'/informationssharing/'.$newref;
				if (!$error && file_exists($dirsource))
				{
					...
					if (@rename($dirsource, $dirdest))
					{
						...
						$listoffiles = dol_dir_list($conf->digiriskdolibarr->dir_output.'/informationssharing/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'INFORMATIONSSHARING_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'INFORMATIONSSHARING_CLOSE');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'INFORMATIONSSHARING_REOPEN');
	}
	...
	public function getLinesArray()
	{
		...
		$objectline = new InformationsSharingLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_informationssharing = '.$this->id));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->DIGIRISKDOLIBARR_INFORMATIONSSHARING_ADDON)) {
			$conf->global->DIGIRISKDOLIBARR_INFORMATIONSSHARING_ADDON = 'mod_informationssharing_standard';
		}
		...
		if (!empty($conf->global->DIGIRISKDOLIBARR_INFORMATIONSSHARING_ADDON))
		{
			...
			$file = $conf->global->DIGIRISKDOLIBARR_INFORMATIONSSHARING_ADDON.".php";
			$classname = $conf->global->DIGIRISKDOLIBARR_INFORMATIONSSHARING_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_informationssharing';

			if ($this->modelpdf) {
				$modele = $this->modelpdf;
			} elseif (!empty($conf->global->INFORMATIONSSHARING_ADDON_PDF)) {
				$modele = $conf->global->INFORMATIONSSHARING_ADDON_PDF;
			}
		}
