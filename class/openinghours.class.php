...
class Openinghours extends CommonObject
{
	 ...
	 */
	//public $table_element_line = 'digiriskdolibarr_openinghoursline';

	/**
	 * @var int    Field with ID of parent key if this object has a parent
	 */
	//public $fk_element = 'fk_openinghours';

	/**
	 * @var int    Name of subtable class that manage subtable lines
	 */
	//public $class_element_line = 'Openinghoursline';
	 ...
	 */
	//protected $childtablesoncascade = array('digiriskdolibarr_openinghoursdet');

	/**
	 * @var OpeninghoursLine[]     Array of subtable lines
	 */
	//public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->digiriskdolibarr->openinghours->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->digiriskdolibarr->openinghours->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->digiriskdolibarr->openinghours->openinghours_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			{
				// Call trigger
				$result = $this->call_trigger('OPENINGHOURS_VALIDATE', $user);
				if ($result < 0) $error++;
				// End call triggers
			}
		}
		...
		if (!$error)
		{
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref))
			{
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'openinghours/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'openinghours/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				...
				$dirsource = $conf->digiriskdolibarr->dir_output.'/openinghours/'.$oldref;
				$dirdest = $conf->digiriskdolibarr->dir_output.'/openinghours/'.$newref;
				if (!$error && file_exists($dirsource))
				{
					...
					if (@rename($dirsource, $dirdest))
					{
						...
						$listoffiles = dol_dir_list($conf->digiriskdolibarr->dir_output.'/openinghours/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'OPENINGHOURS_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'OPENINGHOURS_CLOSE');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'OPENINGHOURS_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = '<u>'.$langs->trans("Openinghours").'</u>';
		$label .= '<br>';
		...
		$url = dol_buildpath('/digiriskdolibarr/openinghours_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip))
		{
			...
			{
				$label = $langs->trans("ShowOpeninghours");
		...
		$hookmanager->initHooks(array('openinghoursdao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new OpeninghoursLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_openinghours = '.$this->id));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->DIGIRISKDOLIBARR_OPENINGHOURS_ADDON)) {
			$conf->global->DIGIRISKDOLIBARR_OPENINGHOURS_ADDON = 'mod_openinghours_standard';
		}
		...
		if (!empty($conf->global->DIGIRISKDOLIBARR_OPENINGHOURS_ADDON))
		{
			...
			$file = $conf->global->DIGIRISKDOLIBARR_OPENINGHOURS_ADDON.".php";
			$classname = $conf->global->DIGIRISKDOLIBARR_OPENINGHOURS_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_openinghours';

			if ($this->modelpdf) {
				$modele = $this->modelpdf;
			} elseif (!empty($conf->global->OPENINGHOURS_ADDON_PDF)) {
				$modele = $conf->global->OPENINGHOURS_ADDON_PDF;
			}
		}
...
/**
 * Class OpeninghoursLine. You can also remove this and generate a CRUD class for lines objects.
 */
class OpeninghoursLine
{
	// To complete with content of an object OpeninghoursLine
	// We should have a field rowid, fk_openinghours and position
