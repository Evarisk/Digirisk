	...
	if ($action == 'builddoc' && $permissiontoadd) {
		...
		if (is_numeric(GETPOST('model', 'alpha'))) {
			...
			if ($id > 0) {
				...
				$result = $object->generateDocument('informationssharing_A4_odt', $outputlangs, $hidedetails, $hidedesc, $hideref, $moreparams);
				if ($result <= 0) {
					setEventMessages($object->error, $object->errors, 'errors');
					$action = '';
				} else {
...
if ((empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	dol_fiche_head($head, 'elementInformationssharing', $langs->trans("DigiriskElement"), -1, $object->picto);
	...
	if ($action != 'presend')
	{
		...
		if ($includedocgeneration) {
			...
			print $formfile->showdocuments($modulepart,$dir_files, $filedir, $urlsource, $genallowed, $delallowed, $conf->global->DIGIRISKDOLIBARR_INFORMATIONSSHARING_DEFAULT_MODEL, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang, '');
		}
