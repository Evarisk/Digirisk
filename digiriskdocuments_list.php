...
/**
 *   	\file       digiriskdocuments_list.php
 *		\ingroup    digiriskdolibarr
 *		\brief      List page for digiriskdocuments
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'digiriskdocumentslist'; // To manage different context of search
...
$hookmanager->initHooks(array('digiriskdocumentslist')); // Note that conf->hooks_modules contains array
...
$permissiontoadd = $user->rights->digiriskdolibarr->digiriskdocuments->write;
$permissiontodelete = $user->rights->digiriskdolibarr->digiriskdocuments->delete;
...
$help_url = '';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("DigiriskDocumentss"));
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page)
{
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/digiriskdolibarr/digiriskdocuments_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/digiriskdolibarr/digiriskdocuments_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
