...
require_once DOL_DOCUMENT_ROOT."/custom/digiriskdolibarr/class/listingrisksphoto.class.php";
...
elseif ($action == 'setdoc')
{
	...
	$tmpobjectkey = preg_replace('/(_odt)/', '', $value);
	$constforval = "DIGIRISKDOLIBARR_LISTINGRISKSPHOTO_DEFAULT_MODEL";
	$type = 'listingrisksphoto';
	$label = '';
...
} elseif ($action == 'setmod')
{
	...
	$tmpobjectkey = 'listingrisksphoto';
...
dol_fiche_head($head, 'listingrisksphoto', '', -1, "digiriskdolibarr@digiriskdolibarr");
...
print load_fiche_titre($langs->trans("DigiriskListingRisksPhotoNumberingModule"), '', '');
		...
		if (is_resource($handle))
		{
			...
			while (($file = readdir($handle)) !== false )
			{
				...
				if (!is_dir($dir.$file) || (substr($file, 0, 1) <> '.' && substr($file, 0, 3) <> 'CVS'))
				{
					...
					// preg_match('/^listingrisksphoto/', $file)
					// For compatibility
					...
					if (!class_exists($classname) && is_readable($dir.$filebis) && (preg_match('/mod_/', $filebis) || preg_match('/mod_/', $classname)) && substr($filebis, dol_strlen($filebis) - 3, 3) == 'php' && preg_match('/listingrisksphoto/i', $classname))
					{
						...
						if ($module->isEnabled())
						{
							...
							if ($conf->global->DIGIRISKDOLIBARR_LISTINGRISKSPHOTO_ADDON == $file || $conf->global->DIGIRISKDOLIBARR_LISTINGRISKSPHOTO_ADDON.'.php' == $file)
							{
							...
							$listingrisksphoto = new DigiriskElement($db);
							$listingrisksphoto->initAsSpecimen();

							// Example for standard invoice
							$htmltooltip = '';
							$htmltooltip .= ''.$langs->trans("Version").': <b>'.$module->getVersion().'</b><br>';
							$listingrisksphoto->type = 0;
							$nextval = $module->getNextValue($listingrisksphoto);
							if ("$nextval" != $langs->trans("NotAvailable")) {  // Keep " on nextval
								$htmltooltip .= $langs->trans("NextValueForListingRisksPhoto").': ';
								if ($nextval) {
							...
							if ($conf->global->DIGIRISKDOLIBARR_LISTINGRISKSPHOTO_ADDON.'.php' == $file)  // If module is the one used, we show existing errors
							{
...
print load_fiche_titre($langs->trans("DigiriskTemplateDocumentListingRisksPhoto"), '', '');
				...
				if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file) && preg_match('/listing_risks_photo/i', $file) && preg_match('/odt/i', $file))
				{
					if (file_exists($dir.'/'.$file))
					{
						...
						if ($modulequalified)
						{
							...
							// Default
							print '<td class="center">';
							if ($conf->global->DIGIRISKDOLIBARR_LISTINGRISKSPHOTO_DEFAULT_MODEL == "$name")
							{
