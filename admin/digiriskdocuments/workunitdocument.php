...
/**
 * \file    digiriskdolibarr/admin/workunitdocument.php
 * \ingroup digiriskdolibarr
 * \brief   Digiriskdolibarr workunitdocument page.
 */
...
{
	$maskconstworkunitdocument = GETPOST('maskconstworkunitdocument', 'alpha');
	$maskworkunitdocument      = GETPOST('maskworkunitdocument', 'alpha');

	if ($maskconstworkunitdocument) $res = dolibarr_set_const($db, $maskconstworkunitdocument, $maskworkunitdocument, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'set')
{
	...
	if ( $value == 'workunitdocument_odt' ) {
		$description = "DIGIRISKDOLIBARR_".strtoupper($type)."_ADDON_ODT_PATH";
	} elseif ( $value == 'workunitdocument_custom_odt' ) {
...
$help_url  = 'FR:Module_DigiriskDolibarr#L.27onglet_Fiche_de_d.27Unit.C3.A9_de_travail';
...
dol_fiche_head($head, 'workunitdocument', '', -1, "digiriskdolibarr@digiriskdolibarr");

/*
 *  Numbering module
 */

print load_fiche_titre($langs->trans("DigiriskWorkUnitDocumentNumberingModule"), '', '');
			...
			if (!is_dir($dir.$file) || (substr($file, 0, 1) <> '.' && substr($file, 0, 3) <> 'CVS'))
			{
				...
				if (!class_exists($classname) && is_readable($dir.$filebis) && (preg_match('/mod_/', $filebis) || preg_match('/mod_/', $classname)) && substr($filebis, dol_strlen($filebis) - 3, 3) == 'php')
				{
					...
					if ($module->isEnabled())
					{
						...
						if ($conf->global->DIGIRISKDOLIBARR_WORKUNITDOCUMENT_ADDON == $file || $conf->global->DIGIRISKDOLIBARR_WORKUNITDOCUMENT_ADDON.'.php' == $file)
						{
						...
						if ($conf->global->DIGIRISKDOLIBARR_WORKUNITDOCUMENT_ADDON.'.php' == $file)  // If module is the one used, we show existing errors
						{
...
if (is_dir($dir))
{
	...
	if (is_resource($handle))
	{
		...
		{
			if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file) && preg_match('/workunitdocument/i', $file) && preg_match('/odt/i', $file))
			{
