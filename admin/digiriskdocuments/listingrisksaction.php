...
/**
 * \file    digiriskdolibarr/admin/listingrisksaction.php
 * \ingroup digiriskdolibarr
 * \brief   Digiriskdolibarr listingrisksaction page.
 */
...
{
	$maskconstlistingrisksaction = GETPOST('maskconstlistingrisksaction', 'alpha');
	$masklistingrisksaction      = GETPOST('masklistingrisksaction', 'alpha');

	if ($maskconstlistingrisksaction) $res = dolibarr_set_const($db, $maskconstlistingrisksaction, $masklistingrisksaction, 'chaine', 0, '', $conf->entity);
...
elseif ($action == 'set')
{
	...
	if ( $value == 'listingrisksaction_odt' ) {
		$description = "DIGIRISKDOLIBARR_".strtoupper($type)."_ADDON_ODT_PATH";
	} elseif ( $value == 'listingrisksaction_custom_odt' ) {
...
$help_url  = 'FR:Module_DigiriskDolibarr#L.27onglet_listings_des_actions_correctives';
...
dol_fiche_head($head, 'listingrisksaction', '', -1, "digiriskdolibarr@digiriskdolibarr");

/*
 *  Numbering module
 */

print load_fiche_titre($langs->trans("DigiriskListingRisksActionNumberingModule"), '', '');
			...
			if (!is_dir($dir.$file) || (substr($file, 0, 1) <> '.' && substr($file, 0, 3) <> 'CVS'))
			{
				...
				if (!class_exists($classname) && is_readable($dir.$filebis) && (preg_match('/mod_/', $filebis) || preg_match('/mod_/', $classname)) && substr($filebis, dol_strlen($filebis) - 3, 3) == 'php')
				{
					...
					if ($module->isEnabled())
					{
						...
						if ($conf->global->DIGIRISKDOLIBARR_LISTINGRISKSACTION_ADDON == $file || $conf->global->DIGIRISKDOLIBARR_LISTINGRISKSACTION_ADDON.'.php' == $file)
						{
						...
						if ($conf->global->DIGIRISKDOLIBARR_LISTINGRISKSACTION_ADDON.'.php' == $file)  // If module is the one used, we show existing errors
						{
...
print load_fiche_titre($langs->trans("DigiriskTemplateDocumentListingRisksAction"), '', '');
...
if (is_dir($dir))
{
	...
	if (is_resource($handle))
	{
		...
		{
			if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file) && preg_match('/listingrisksaction/i', $file) && preg_match('/odt/i', $file))
			{
				if (file_exists($dir.'/'.$file))
				{
					...
					if ($modulequalified)
					{
						...
						// Default
						print '<td class="center">';
						if ($conf->global->DIGIRISKDOLIBARR_LISTINGRISKSACTION_DEFAULT_MODEL == $name)
						{
