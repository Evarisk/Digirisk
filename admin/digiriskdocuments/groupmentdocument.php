...
/**
 * \file    digiriskdolibarr/admin/groupmentdocument.php
...
dol_fiche_head($head, 'groupmentdocument', '', -1, "digiriskdolibarr@digiriskdolibarr");
			...
			if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file) && preg_match('/groupmentdocument/i', $file) && preg_match('/odt/i', $file))
			{
				if (file_exists($dir.'/'.$file))
				{
					...
					if ($modulequalified)
					{
						...
						// Default
						print '<td class="center">';
						if ($conf->global->DIGIRISKDOLIBARR_GROUPMENT_DOCUMENT_DEFAULT_MODEL == $name)
						{
