...
require_once DOL_DOCUMENT_ROOT."/custom/digiriskdolibarr/class/listingrisksaction.class.php";
...
elseif ($action == 'setdoc')
{
	...
	$tmpobjectkey = preg_replace('/(_odt)/', '', $value);
	$constforval = "DIGIRISKDOLIBARR_LISTINGRISKSACTION_DEFAULT_MODEL";
	$type = 'listingrisksaction';
	$label = '';
...
} elseif ($action == 'setmod')
{
	...
	$tmpobjectkey = 'listingrisksaction';
		...
		if (is_resource($handle))
		{
			...
			while (($file = readdir($handle)) !== false )
			{
				...
				if (!is_dir($dir.$file) || (substr($file, 0, 1) <> '.' && substr($file, 0, 3) <> 'CVS'))
				{
					...
					// preg_match('/^listingrisksaction/', $file)
					// For compatibility
					...
					if (!class_exists($classname) && is_readable($dir.$filebis) && (preg_match('/mod_/', $filebis) || preg_match('/mod_/', $classname)) && substr($filebis, dol_strlen($filebis) - 3, 3) == 'php' && preg_match('/listingrisksaction/i', $classname))
					{
						...
						if ($module->isEnabled())
						{
							...
							$listingrisksaction = new DigiriskElement($db);
							$listingrisksaction->initAsSpecimen();
