...
require_once DOL_DOCUMENT_ROOT."/custom/digiriskdolibarr/class/informationssharing.class.php";
...
} elseif ($action == 'setmod')
{
	...
	$tmpobjectkey = 'informationssharing';
...
dol_fiche_head($head, 'informationssharing', '', -1, "digiriskdolibarr@digiriskdolibarr");
...
print load_fiche_titre($langs->trans("DigiriskInformationsSharingNumberingModule"), '', '');
		...
		if (is_resource($handle))
		{
			...
			while (($file = readdir($handle)) !== false )
			{
				...
				if (!is_dir($dir.$file) || (substr($file, 0, 1) <> '.' && substr($file, 0, 3) <> 'CVS'))
				{
					...
					// preg_match('/^informationssharing/', $file)
					// For compatibility
					...
					if (!class_exists($classname) && is_readable($dir.$filebis) && (preg_match('/mod_/', $filebis) || preg_match('/mod_/', $classname)) && substr($filebis, dol_strlen($filebis) - 3, 3) == 'php' && preg_match('/informationssharing/i', $classname))
					{
						...
						if ($module->isEnabled())
						{
							...
							if ($conf->global->DIGIRISKDOLIBARR_INFORMATIONSSHARING_ADDON == $file || $conf->global->DIGIRISKDOLIBARR_INFORMATIONSSHARING_ADDON.'.php' == $file)
							{
							...
							$informationssharing->initAsSpecimen();
							...
							if ($conf->global->DIGIRISKDOLIBARR_INFORMATIONSSHARING_ADDON.'.php' == $file)  // If module is the one used, we show existing errors
							{
...
/*
 *  Documents models for Informations Sharing
 */

print load_fiche_titre($langs->trans("DigiriskTemplateDocumentInformationsSharing"), '', '');
...
foreach ($dirmodels as $reldir)
{
	...
	if (is_dir($dir))
	{
		...
		if (is_resource($handle))
		{
			...
			{
				if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file) && preg_match('/informationssharing/i', $file) && preg_match('/odt/i', $file))
				{
