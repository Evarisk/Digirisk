...
require_once DOL_DOCUMENT_ROOT."/custom/digiriskdolibarr/class/firepermit.class.php";
...
elseif ($action == 'setdoc')
{
	...
	$tmpobjectkey = preg_replace('/(_odt)/', '', $value);
	$constforval = "DIGIRISKDOLIBARR_FIREPERMIT_DEFAULT_MODEL";
	$type = 'firepermit';
	$label = '';
		...
		if (is_resource($handle))
		{
			...
			while (($file = readdir($handle)) !== false )
			{
				...
				if (!is_dir($dir.$file) || (substr($file, 0, 1) <> '.' && substr($file, 0, 3) <> 'CVS'))
				{
					...
					if (!class_exists($classname) && is_readable($dir.$filebis) && (preg_match('/mod_/', $filebis) || preg_match('/mod_/', $classname)) && substr($filebis, dol_strlen($filebis) - 3, 3) == 'php' && preg_match('/firepermit/i', $classname))
					{
						...
						if ($module->isEnabled())
						{
							...
							$firepermit->initAsSpecimen();

							// Example for standard invoice
							$htmltooltip = '';
							$htmltooltip .= ''.$langs->trans("Version").': <b>'.$module->getVersion().'</b><br>';
							$firepermit->type = 0;
							$nextval = $module->getNextValue($mysoc, $firepermit);
