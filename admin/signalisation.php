...
elseif ($action == 'setdoc')
{
	...
	$tmpobjectkey = preg_replace('/(_odt)/', '', $value);
	$constforval = "DIGIRISKDOLIBARR_SIGNALISATION_DEFAULT_MODEL";
	$type = 'signalisation';
	$label = '';
...
dol_fiche_head($head, 'signalisation', '', -1, "digiriskdolibarr@digiriskdolibarr");
...
print load_fiche_titre($langs->trans("DigiriskSignalisationNumberingModule"), '', '');
		...
		if (is_resource($handle))
		{
			...
			while (($file = readdir($handle)) !== false )
			{
				...
				if (!is_dir($dir.$file) || (substr($file, 0, 1) <> '.' && substr($file, 0, 3) <> 'CVS'))
				{
					...
					if (!class_exists($classname) && is_readable($dir.$filebis) && (preg_match('/mod_/', $filebis) || preg_match('/mod_/', $classname)) && substr($filebis, dol_strlen($filebis) - 3, 3) == 'php' && preg_match('/signalisation/i', $classname))
					{
						...
						if ($module->isEnabled())
						{
							...
							if ($conf->global->DIGIRISKDOLIBARR_SIGNALISATION_ADDON == $file || $conf->global->DIGIRISKDOLIBARR_SIGNALISATION_ADDON.'.php' == $file)
							{
							...
							$signalisation = new DigiriskElement($db);
							$signalisation->initAsSpecimen();

							// Example for standard invoice
							$htmltooltip = '';
							$htmltooltip .= ''.$langs->trans("Version").': <b>'.$module->getVersion().'</b><br>';
							$signalisation->type = 0;
							$nextval = $module->getNextValue($signalisation);
							...
							if ($conf->global->DIGIRISKDOLIBARR_SIGNALISATION_ADDON.'.php' == $file)  // If module is the one used, we show existing errors
							{
...
print load_fiche_titre($langs->trans("DigiriskTemplateDocumentSignalisation"), '', '');
...
foreach ($dirmodels as $reldir)
{
	...
	if (is_dir($dir))
	{
		...
		if (is_resource($handle))
		{
			...
			{
				if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file) && preg_match('/signalisation_A4/i', $file) && preg_match('/odt/i', $file))
				{
					if (file_exists($dir.'/'.$file))
					{
						...
						if ($modulequalified)
						{
							...
							// Default
							print '<td class="center">';
							if ($conf->global->DIGIRISKDOLIBARR_SIGNALISATION_DEFAULT_MODEL == "$name")
							{
