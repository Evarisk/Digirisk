...
require_once DOL_DOCUMENT_ROOT."/custom/digiriskdolibarr/class/preventionplan.class.php";
...
elseif ($action == 'setdoc')
{
	...
	$tmpobjectkey = preg_replace('/(_odt)/', '', $value);
	$constforval = "DIGIRISKDOLIBARR_PREVENTIONPLAN_DEFAULT_MODEL";
	$type = 'preventionplan';
	$label = '';
		...
		if (is_resource($handle))
		{
			...
			while (($file = readdir($handle)) !== false )
			{
				...
				if (!is_dir($dir.$file) || (substr($file, 0, 1) <> '.' && substr($file, 0, 3) <> 'CVS'))
				{
					...
					// preg_match('/^preventionplan/', $file)
					// For compatibility
					...
					if (!class_exists($classname) && is_readable($dir.$filebis) && (preg_match('/mod_/', $filebis) || preg_match('/mod_/', $classname)) && substr($filebis, dol_strlen($filebis) - 3, 3) == 'php' && preg_match('/preventionplan/i', $classname))
					{
						...
						if ($module->isEnabled())
						{
							...
							$tmp = $module->prefixpreventionplan;
							...
							$preventionplan->initAsSpecimen();

							// Example for standard invoice
							$htmltooltip = '';
							$htmltooltip .= ''.$langs->trans("Version").': <b>'.$module->getVersion().'</b><br>';
							$preventionplan->type = 0;
							$nextval = $module->getNextValue($mysoc, $preventionplan);
...
/*
 *  Documents models for Prevention Plan
 */
