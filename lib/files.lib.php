 ...
 */
function digiriskdol_basename($pathfile)
{
 ...
 *  @see digiriskdol_dir_list_in_database()
 */
function digiriskdol_dir_list($path, $types = "all", $recursive = 0, $filter = "", $excludefilter = null, $sortcriteria = "name", $sortorder = SORT_ASC, $mode = 0, $nohook = 0, $relativename = "", $donotfollowsymlinks = 0)
{
			...
			while (false !== ($file = readdir($dir)))        // $file is always a basename (into directory $newpath)
			{
				...
				if ($qualified)
				{
					...
					if ($isdir && (($types == "directories") || ($types == "all") || $recursive))
					{
						...
						{
							if ($loaddate || $sortcriteria == 'date') $filedate = digiriskdol_filemtime($path."/".$file);
							if ($loadsize || $sortcriteria == 'size') $filesize = digiriskdol_filesize($path."/".$file);
						...
						if ($recursive)
						{
							if (empty($donotfollowsymlinks) || !is_link($path."/".$file))
							{
								...
								$file_list = array_merge($file_list, digiriskdol_dir_list($path."/".$file, $types, $recursive, $filter, $excludefilter, $sortcriteria, $sortorder, $mode, $nohook, ($relativename != '' ? $relativename.'/' : '').$file, $donotfollowsymlinks));
							}
						}
					}
 ...
 *  @see digiriskdol_dir_list()
 */
function digiriskdol_dir_list_in_database($path, $filter = "", $excludefilter = null, $sortcriteria = "name", $sortorder = SORT_ASC, $mode = 0)
{
	global $conf, $db;
 ...
 *
 * @param	array	$filearray			Array of files get using digiriskdol_dir_list
 * @param	string	$relativedir		Relative dir from DOL_DATA_ROOT
 * @return	void
 */
function digiriskcompleteFileArrayWithDatabaseInfo(&$filearray, $relativedir)
{
	global $conf, $db, $user;

	$filearrayindatabase = digiriskdol_dir_list_in_database($relativedir, '', null, 'name', SORT_ASC);
	...
	if ($modulepart == 'produit' && !empty($conf->global->PRODUCT_USE_OLD_PATH_FOR_PHOTO)) {
		...
		if (!empty($object->id))
		{
			...
			$filearrayindatabase = array_merge($filearrayindatabase, digiriskdol_dir_list_in_database($relativedirold, '', null, 'name', SORT_ASC));
		}
	}
 ...
 * @return 	int				1, 0, 1
 */
function digiriskdol_compare_file($a, $b)
{
	global $sortorder;
	global $sortfield;
 ...
 */
function digiriskdol_is_dir($folder)
{
	$newfolder = digiriskdol_osencode($folder);
 ...
 */
function digiriskdol_is_dir_empty($dir)
{
 ...
 * @return  boolean     			    True or false
 */
function digiriskdol_is_file($pathoffile)
{
	$newpathoffile = digiriskdol_osencode($pathoffile);
 ...
 * @return  boolean     			    True or false
 */
function digiriskdol_is_link($pathoffile)
{
	$newpathoffile = digiriskdol_osencode($pathoffile);
 ...
 * @return  boolean      	   	True or false
 */
function digiriskdol_is_url($url)
{
 ...
 */
function digiriskdol_dir_is_emtpy($folder)
{
	$newfolder = digiriskdol_osencode($folder);
	if (is_dir($newfolder))
	{
 ...
 *  @see digiriskdol_nboflines()
 */
function digiriskdol_count_nb_of_line($file)
{
	$nb = 0;

	$newfile = digiriskdol_osencode($file);
	//print 'x'.$file;
	$fp = fopen($newfile, 'r');
	if ($fp)
	{
 ...
 * @see digiriskdol_print_size()
 */
function digiriskdol_filesize($pathoffile)
{
 ...
 */
function digiriskdol_filemtime($pathoffile)
{
	$newpathoffile = dol_osencode($pathoffile);
 ...
 * @see		dol_copy()
 */
function digiriskdolReplaceInFile($srcfile, $arrayreplacement, $destfile = '', $newmask = 0, $indexdatabase = 0, $arrayreplacementisregex = 0)
{
	global $conf;
	...
	$destexists = digiriskdol_is_file($destfile);
	if (($destfile != $srcfile) && $destexists) return 0;

	$tmpdestfile = $destfile.'.tmp';

	$newpathofsrcfile = digiriskdol_osencode($srcfile);
	$newpathoftmpdestfile = digiriskdol_osencode($tmpdestfile);
	$newpathofdestfile = digiriskdol_osencode($destfile);
	...
	// Rename
	$result = digiriskdol_move($newpathoftmpdestfile, $newpathofdestfile, $newmask, (($destfile == $srcfile) ? 1 : 0), 0, $indexdatabase);
	if (!$result)
	{
 ...
 */
function digiriskdol_copy($srcfile, $destfile, $newmask = 0, $overwriteifexists = 1)
{
	global $conf;
	...
	if (empty($newmask))	// This should no happen
	{
		dol_syslog("Warning: digiriskdol_copy called with empty value for newmask and no default value defined", LOG_WARNING);
		$newmask = '0664';
	}
 ...
 * @see		dol_copy()
 */
function digiriskdolCopyDir($srcfile, $destfile, $newmask, $overwriteifexists, $arrayreplacement = null, $excludesubdir = 0)
{
	global $conf;
	...
	$destexists = digiriskdol_is_dir($destfile);
	//if (! $overwriteifexists && $destexists) return 0;	// The overwriteifexists is for files only, so propagated to digiriskdol_copy only.

	if (!$destexists)
	{
		// We must set mask just before creating dir, becaause it can be set differently by digiriskdol_copy
		umask(0);
	...
	$ossrcfile = digiriskdol_osencode($srcfile);
	$osdestfile = digiriskdol_osencode($destfile);
		...
		while ($file = readdir($dir_handle))
		{
			if ($file != "." && $file != ".." && !is_link($ossrcfile."/".$file))
			{
				...
				else
				{
					...
					$tmpresult = digiriskdol_copy($srcfile."/".$file, $destfile."/".$newfile, $newmask, $overwriteifexists);
				}
				// Set result
 ...
 * Note:
 *  - This function differs from digiriskdol_move_uploaded_file, because it can be called in any context.
 ...
 * @see digiriskdol_move_uploaded_file()
 */
function digiriskdol_move($srcfile, $destfile, $newmask = 0, $overwriteifexists = 1, $testvirus = 0, $indexdatabase = 1)
{
	global $user, $db, $conf;
	$result = false;

	dol_syslog("files.lib.php::dol_move srcfile=".$srcfile." destfile=".$destfile." newmask=".$newmask." overwritifexists=".$overwriteifexists);
	$srcexists = digiriskdol_is_file($srcfile);
	$destexists = digiriskdol_is_file($destfile);
	...
	if ($overwriteifexists || !$destexists)
	{
		...
		if (!$result)
		{
			...
			else digiriskdol_syslog("files.lib.php::dol_move Failed.", LOG_WARNING);
		}
 ...
 */
function digiriskdol_unescapefile($filename)
{
 ...
 */
function digiriskdolCheckVirus($src_file)
{
	global $conf;
 ...
 *  Note:
 *  - This function can be used only into a HTML page context. Use digiriskdol_move if you are outside.
 *  - Test on antivirus is always done (if antivirus set).
 *  - Database of files is NOT updated (this is done by digiriskdol_add_file_process() that calls this function).
 ...
 *  @see    digiriskdol_move()
 */
function digiriskdol_move_uploaded_file($src_file, $dest_file, $allowoverwrite, $disablevirusscan = 0, $uploaderrorcode = 0, $nohook = 0, $varfiles = 'addedfile', $upload_dir = '')
{
	...
	elseif (empty($reshook))
	{
		...
		$src_file_osencoded = digiriskdol_osencode($src_file);
		$file_name_osencoded = digiriskdol_osencode($file_name);
 ...
 *  @see digiriskdol_delete_dir()
 */
function digiriskdol_delete_file($file, $disableglob = 0, $nophperrors = 0, $nohook = 0, $object = null, $allowdotdot = false, $indexdatabase = 1)
{
	...
	else
	{
		...
		$file_osencoded = digiriskdol_osencode($file); // New filename encoded in OS filesystem encoding charset
		if (empty($disableglob) && !empty($file_osencoded))
		{
			...
			else digiriskdol_syslog("No files to delete found", LOG_DEBUG);
		}
		else
		{
			$ok = false;
			...
			if ($ok) digiriskdol_syslog("Removed file ".$file_osencoded, LOG_DEBUG);
			else digiriskdol_syslog("Failed to remove file ".$file_osencoded, LOG_WARNING);
		}
 ...
 *  @see digiriskdol_delete_file() dolCopyDir()
 */
function digiriskdol_delete_dir($dir, $nophperrors = 0)
{
	// Security:
	// We refuse transversal using .. and pipes into filenames.
 ...
 */
function digiriskdol_delete_dir_recursive($dir, $count = 0, $nophperrors = 0, $onlysub = 0, &$countdeleted = 0)
{
		...
		if ($handle = opendir("$dir_osencoded"))
		{
			while (false !== ($item = readdir($handle)))
			{
				...
				if ($item != "." && $item != "..")
				{
					...
					{
						$count = digiriskdol_delete_dir_recursive("$dir/$item", $count, $nophperrors, 0, $countdeleted);
					}
					else
					{
						$result = digiriskdol_delete_file("$dir/$item", 1, $nophperrors);
						$count++;
			...
			if (empty($onlysub))
			{
				$result = digiriskdol_delete_dir($dir, $nophperrors);
				$count++;
				if ($result) $countdeleted++;
 ...
 *  Note that preview image of PDF files is generated when required, by digiriskdol_banner_tab() for example.
 *
 *  @param	object	$object		Object to clean
 *  @return	int					0 if error, 1 if OK
 *  @see digiriskdol_convert_file()
 */
function digiriskdol_delete_preview($object)
{
	global $langs, $conf;
	...
	$refsan = digiriskdol_sanitizeFileName($object->ref);
	$dir = $dir."/".$refsan;
 ...
 */
function digiriskdol_meta_create($object)
{
	global $conf;
	...
	if ($dir)
	{
		...
		if (is_dir($dir))
		{
			...
			DATE=\"" . digiriskdol_print_date($object->date, '')."\"
			NB_ITEMS=\"" . $nblines."\"
			CLIENT=\"" . $client."\"
 ...
 * @return	void
 */
function digiriskdol_init_file_process($pathtoscan = '', $trackid = '')
{
	$listofpaths = array();
	$listofnames = array();
	$listofmimes = array();
	...
	if ($pathtoscan)
	{
		$listoffiles = digiriskdol_dir_list($pathtoscan, 'files');
 ...
 */
function digiriskdol_add_file_process($upload_dir, $allowoverwrite = 0, $donotupdatesession = 0, $varfiles = 'addedfile', $savingdocmask = '', $link = null, $trackid = '', $generatethumbs = 1)
{
	...
	if (!empty($_FILES[$varfiles])) // For view $_FILES[$varfiles]['error']
	{
		...
		if (dol_mkdir($upload_dir) >= 0)
		{
			...
			for ($i = 0; $i < $nbfile; $i++)
			{
				...
				// digiriskdol_sanitizeFileName the file name and lowercase extension
				$info = pathinfo($destfull);
				$destfull = $info['dirname'].'/'.dol_sanitizeFileName($info['filename'].($info['extension'] != '' ? ('.'.strtolower($info['extension'])) : ''));
				$info = pathinfo($destfile);

				$destfile = digiriskdol_sanitizeFileName($info['filename'].($info['extension'] != '' ? ('.'.strtolower($info['extension'])) : ''));

				// We apply digiriskdol_string_nohtmltag also to clean file names (this remove duplicate spaces) because
				// this function is also applied when we make try to download file (by the GETPOST(filename, 'alphanohtml') call).
				$destfile = digiriskdol_string_nohtmltag($destfile);
				$destfull = digiriskdol_string_nohtmltag($destfull);

				// Move file from temp directory to final directory. A .noexe may also be appended on file name.
				$resupload = digiriskdol_move_uploaded_file($TFile['tmp_name'][$i], $destfull, $allowoverwrite, 0, $TFile['error'][$i], 0, $varfiles, $upload_dir);
 ...
 * @return	void
 */
function digiriskdol_remove_file_process($filenb, $donotupdatesession = 0, $donotdeletefile = 1, $trackid = '')
{
	...
	if ($keytodelete >= 0)
	{
		...
		if (empty($donotdeletefile)) $result = digiriskdol_delete_file($pathtodelete, 1); // The delete of ecm database is inside the function digiriskdol_delete_file
		else $result = 0;
		if ($result >= 0)
		{
 ...
 *  Called by digiriskdol_add_file_process when uploading a file and on other cases.
 ...
 */
function digiriskaddFileIntoDatabaseIndex($dir, $file, $fullpathorig = '', $mode = 'uploaded', $setsharekey = 0)
{
	global $db, $user;
 ...
 *	@return		int						<0 if KO, 0 if nothing done, >0 if OK
 */
function digiriskdeleteFilesIntoDatabaseIndex($dir, $file, $mode = 'uploaded')
{
	global $conf, $db, $user;
 ...
 */
function digiriskdol_convert_file($fileinput, $ext = 'png', $fileoutput = '', $page = '')
{
	global $langs;
 ...
 * @return	int						<0 if KO, >0 if OK
 */
function digiriskdol_compress_file($inputfile, $outputfile, $mode = "gz", &$errorstring = null)
{
	global $conf;
 ...
 */
function digiriskdol_uncompress($inputfile, $outputdir)
{
	global $conf, $langs;
 ...
 * @return	int						<0 if KO, >0 if OK
 */
function digiriskdol_compress_dir($inputdir, $outputfile, $mode = "zip", $excludefiles = '', $rootdirinzip = '')
{
	$foundhandler = 0;
 ...
 */
function digiriskdol_most_recent_file($dir, $regexfilter = '', $excludefilter = array('(\.meta|_preview.*\.png)$', '^\.'), $nohook = false, $mode = '')
{
	$tmparray = digiriskdol_dir_list($dir, 'files', 0, $regexfilter, $excludefilter, 'date', SORT_DESC, $mode, $nohook);
	return $tmparray[0];
}
 ...
 * @see restrictedArea()
 */
function digiriskdol_check_secure_access_document($modulepart, $original_file, $entity, $fuser = '', $refname = '', $mode = 'read')
{
	global $conf, $db, $user;
	...
	elseif ($modulepart == 'digiriskdolibarr:legaldisplay' && !empty($conf->digiriskdolibarr->multidir_output[$entity]))
	{
		...
		$original_file = $conf->digiriskdolibarr->multidir_output[$entity]. '/' . $submodulepart .'/'.$original_file;
		$sqlprotectagainstexternals = "SELECT fk_soc as fk_soc FROM ".MAIN_DB_PREFIX."commande WHERE ref='".$db->escape($refname)."' AND entity IN (".getEntity('order').")";
	}
	...
	else
	{
		...
		if (preg_match('/^([a-z]+)_user_temp$/i', $modulepart, $reg))
		{
			...
			{
				dol_print_error('', 'Error call digiriskdol_check_secure_access_document with not supported value for modulepart parameter ('.$modulepart.')');
				exit;
			}
		...
		else
		{
			...
			{
				dol_print_error('', 'Error call MAAAAdol_check_secure_access_document with not supported value for modulepart parameter ('.$modulepart.'). The module for this modulepart value may not be activated.');
				exit;
			}
 ...
 * @return void
 */
function digiriskdol_filecache($directory, $filename, $object)
{
	if (!dol_is_dir($directory)) digiriskdol_mkdir($directory);
 ...
 */
function digiriskdol_cache_refresh($directory, $filename, $cachetime)
{
	$now = digiriskdol_now();
	$cachefile = $directory.$filename;
	$refresh = !file_exists($cachefile) || ($now - $cachetime) > digiriskdol_filemtime($cachefile);
	return $refresh;
}
 ...
 */
function digiriskdol_readcachefile($directory, $filename)
{
	$cachefile = $directory.$filename;
 ...
 */
function digiriskgetFilesUpdated(&$file_list, SimpleXMLElement $dir, $path = '', $pathref = '', &$checksumconcat = array())
{
	global $conffile;
