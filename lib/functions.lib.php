 ...
 */
function digiriskgetDoliDBInstance($type, $host, $user, $pass, $name, $port)
{
 ...
 */
function digiriskgetEntity($element, $shared = 1, $currentobject = null)
{
	global $conf, $mc;
 ...
 */
function digirisksetEntity($currentobject)
{
	global $conf, $mc;
 ...
 * @return	array Check function digiriskdocumentation
 */
function digiriskgetBrowserInfo($user_agent)
{
 ...
 */
function digiriskGETPOSTISSET($paramname)
{
	$isset = 0;
 ...
 */
function digiriskGETPOST($paramname, $check = 'alphanohtml', $method = 0, $filter = null, $options = null, $noreplace = 0)
{
						...
						if (isset($user->default_values[$relativepathstring]['createform']))
						{
							foreach ($user->default_values[$relativepathstring]['createform'] as $defkey => $defval)
							{
								...
								if ($defkey != '_noquery_')
								{
									...
									$tmpqueryarraywehave = explode('&', digiriskdol_string_nohtmltag($_SERVER['QUERY_STRING']));
									$foundintru = 0;
								...
								foreach ($user->default_values[$relativepathstring]['sortorder'] as $defkey => $defval)
								{
									...
									if ($qualified)
									{
										...
										foreach ($user->default_values[$relativepathstring]['sortorder'][$defkey] as $key => $val)
										{
											...
											{
												$out .= digiriskdol_string_nospecial($key, '', $forbidden_chars_to_replace);
											}
											if ($paramname == 'sortorder')
											{
												$out .= digiriskdol_string_nospecial($val, '', $forbidden_chars_to_replace);
											}
										}
							...
							foreach ($user->default_values[$relativepathstring]['filters'] as $defkey => $defval)	// $defkey is a querystring like 'a=b&c=d', $defval is key of user
							{
								...
								if ($qualified)
								{
									if (isset($_POST['sall']) || isset($_POST['search_all']) || isset($_GET['sall']) || isset($_GET['search_all']))
									{
										...
										{
											$forbidden_chars_to_replace = array(" ", "'", "/", "\\", ":", "*", "?", "\"", "<", ">", "|", "[", "]", ";", "="); // we accept _, -, . and ,
											$out = digiriskdol_string_nospecial($user->default_values[$relativepathstring]['filters'][$defkey][$paramname], '', $forbidden_chars_to_replace);
										}
									}
									else
									{
	...
	if (!is_array($out) && empty($_POST[$paramname]) && empty($noreplace))
	{
		...
		while (preg_match('/__([A-Z0-9]+_?[A-Z0-9]+)__/i', $out, $reg) && ($loopnb < $maxloop))    // Detect '__ABCDEF__' as key 'ABCDEF' and '__ABC_DEF__' as key 'ABC_DEF'. Detection is also correct when 2 vars are side by side.
		{
			...
			if ($reg[1] == 'DAY') { $tmp = digiriskdol_getdate(dol_now(), true); $newout = $tmp['mday']; }
			elseif ($reg[1] == 'MONTH') { $tmp = digiriskdol_getdate(dol_now(), true); $newout = $tmp['mon']; }
			elseif ($reg[1] == 'YEAR') { $tmp = digiriskdol_getdate(dol_now(), true); $newout = $tmp['year']; }
			elseif ($reg[1] == 'PREVIOUS_DAY') { $tmp = digiriskdol_getdate(dol_now(), true); $tmp2 = digiriskdol_get_prev_day($tmp['mday'], $tmp['mon'], $tmp['year']); $newout = $tmp2['day']; }
			elseif ($reg[1] == 'PREVIOUS_MONTH') { $tmp = digiriskdol_getdate(dol_now(), true); $tmp2 = digiriskdol_get_prev_month($tmp['mon'], $tmp['year']); $newout = $tmp2['month']; }
			elseif ($reg[1] == 'PREVIOUS_YEAR') { $tmp = digiriskdol_getdate(dol_now(), true); $newout = ($tmp['year'] - 1); }
			elseif ($reg[1] == 'NEXT_DAY') { $tmp = digiriskdol_getdate(dol_now(), true); $tmp2 = digiriskdol_get_next_day($tmp['mday'], $tmp['mon'], $tmp['year']); $newout = $tmp2['day']; }
			elseif ($reg[1] == 'NEXT_MONTH') { $tmp = digiriskdol_getdate(dol_now(), true); $tmp2 = digiriskdol_get_next_month($tmp['mon'], $tmp['year']); $newout = $tmp2['month']; }
			elseif ($reg[1] == 'NEXT_YEAR') { $tmp = digiriskdol_getdate(dol_now(), true); $newout = ($tmp['year'] + 1); }
	...
	switch ($check)
	{
		...
		case 'array':
			...
			break;
		case 'nohtml':
			$out = digiriskdol_string_nohtmltag($out, 0);
			break;
		...
		case 'restricthtml':		// Recommended for most html textarea
			$out = digiriskdol_string_onlythesehtmltags($out, 0);
			break;
		case 'custom':
...
if (!function_exists('dol_getprefix'))
{
	 ...
	 */
	function digiriskdol_getprefix($mode = '')
	{
		global $conf;
		...
		if ($mode == 'email')
		{
			...
			return digiriskdol_hash(DOL_DOCUMENT_ROOT.DOL_URL_ROOT, '3');
		}
		...
		{
			return digiriskdol_hash($_SERVER["SERVER_NAME"].$_SERVER["DOCUMENT_ROOT"].DOL_DOCUMENT_ROOT.DOL_URL_ROOT, '3');
		}
 ...
 *  To link to a module file from a core file, then this function digiriskcan be used (call by hook / trigger / speciales pages)
 *
 * 	@param	string	$relpath	Relative path to file (Ie: mydir/myfile, ../myfile, ...)
 * 	@param	string	$classname	Class name (deprecated)
 *  @return bool                True if load is a success, False if it fails
 */
function digiriskdol_include_once($relpath, $classname = '')
{
	...
	$fullpath = digiriskdol_buildpath($relpath);
 ...
 */
function digiriskdol_buildpath($path, $type = 0, $returnemptyifnotfound = 0)
{
	global $conf;
 ...
 */
function digiriskdol_clone($object, $native = 0)
{
	if (empty($native))
	{
 ...
 * 								'' = function digiriskused to define a size for truncation
 * 								'width' = function digiriskis used to define a width
 *	@return int					New size after optimizing
 */
function digiriskdol_size($size, $type = '')
{
	global $conf;
 ...
 *
 * 	@see        	dol_string_nospecial(), digiriskdol_string_unaccent(), digiriskdol_sanitizePathName()
 */
function digiriskdol_sanitizeFileName($str, $newstr = '_', $unaccent = 1)
{
	// List of special chars for filenames in windows are defined on page https://docs.microsoft.com/en-us/windows/win32/fileio/naming-a-file
	// Char '>' '<' '|' '$' and ';' are special chars for shells.
	// Char '/' and '\' are file delimiters.
	$filesystem_forbidden_chars = array('<', '>', '/', '\\', '?', '*', '|', '"', ':', '°', '$', ';');
	return digiriskdol_string_nospecial($unaccent ?dol_string_unaccent($str) : $str, $newstr, $filesystem_forbidden_chars);
}
 ...
 *	@return string          		String cleaned (a-zA-Z_)
 *
 * 	@see        	dol_string_nospecial(), digiriskdol_string_unaccent(), digiriskdol_sanitizeFileName()
 */
function digiriskdol_sanitizePathName($str, $newstr = '_', $unaccent = 1)
{
	$filesystem_forbidden_chars = array('<', '>', '?', '*', '|', '"', '°');
	return digiriskdol_string_nospecial($unaccent ? digiriskdol_string_unaccent($str) : $str, $newstr, $filesystem_forbidden_chars);
}

/**
 *	Clean a string from all accent characters to be used as ref, login or by digiriskdol_sanitizeFileName
 *
 *	@param	string	$str			String to clean
 *	@return string   	       		Cleaned string
 *
 * 	@see    		dol_sanitizeFilename(), digiriskdol_string_nospecial()
 */
function digiriskdol_string_unaccent($str)
{
	if (utf8_check($str))
	{
 ...
 *  This is a more complete function digiriskthan digiriskdol_sanitizeFileName.
 *
 *	@param	string	$str            	String to clean
 ...
 * 	@return string          			Cleaned string
 *
 * 	@see    		dol_sanitizeFilename(), digiriskdol_string_unaccent()
 */
function digiriskdol_string_nospecial($str, $newstr = '_', $badcharstoreplace = '')
{
	$forbidden_chars_to_replace = array(" ", "'", "/", "\\", ":", "*", "?", "\"", "<", ">", "|", "[", "]", ",", ";", "=", '°'); // more complete than digiriskdol_sanitizeFileName
 ...
 */
function digiriskdol_escape_js($stringtoescape, $mode = 0, $noescapebackslashn = 0)
{
 ...
 */
function digiriskdol_escape_json($stringtoescape)
{
 ...
 *  @see		dol_string_nohtmltag(), digiriskdol_string_nospecial(), digiriskdol_string_unaccent()
 */
function digiriskdol_escape_htmltag($stringtoescape, $keepb = 0, $keepn = 0, $keepmoretags = '', $escapeonlyhtmltags = 0)
{
 ...
 */
function digiriskdol_strtolower($utf8_string)
{
 ...
 * @return 	string							String converted
 */
function digiriskdol_strtoupper($utf8_string)
{
 ...
 *  This function digiriskworks only if syslog module is enabled.
 * 	This must not use any call to other function digiriskcalling digiriskdol_syslog (avoid infinite loop).
 *
 ...
 *  @return	void
 */
function digiriskdol_syslog($message, $level = LOG_INFO, $ident = 0, $suffixinfilename = '', $restricttologhandler = '', $logcontext = null)
{
	...
	if (!empty($message))
	{
		...
		{
			$conf->logbuffer[] = digiriskdol_print_date(time(), "%Y-%m-%d %H:%M:%S")." ".$logLevels[$level]." ".$message;
		}
 ...
 *
 *	@param	array	$links				Array of tabs. Currently initialized by calling a function digiriskxxx_admin_prepare_head
 *	@param	string	$active     		Active tab name (document', 'info', 'ldap', ....)
 *	@param  string	$title      		Title
 *	@param  int		$notab				-1 or 0=Add tab header, 1=no tab header (if you set this to 1, using digiriskdol_fiche_end() to close tab is not required), -2=Add tab header with no seaparation under tab (to start a tab just after)
 * 	@param	string	$picto				Add a picto on tab title
 *	@param	int		$pictoisfullpath	If 1, image path is a full path. If you set this to 1, you can use url returned by digiriskdol_buildpath('/mymodyle/img/myimg.png',1) for $picto.
 *  @param	string	$morehtmlright		Add more html content on right of tabs title
 *  @param	string	$morecss			More Css
 *  @param	int		$limittoshow		Limit number of tabs to show. Use 0 to use automatic default value.
 * 	@return	void
 */
function digiriskdol_fiche_head($links = array(), $active = '0', $title = '', $notab = 0, $picto = '', $pictoisfullpath = 0, $morehtmlright = '', $morecss = '', $limittoshow = 0)
{
	print digiriskdol_get_fiche_head($links, $active, $title, $notab, $picto, $pictoisfullpath, $morehtmlright, $morecss, $limittoshow);
}
 ...
 * 	@return	string
 */
function digiriskdol_get_fiche_head($links = array(), $active = '', $title = '', $notab = 0, $picto = '', $pictoisfullpath = 0, $morehtmlright = '', $morecss = '', $limittoshow = 0)
{
 ...
 *  @return	void
 */
function digiriskdol_fiche_end($notab = 0)
{
	print digiriskdol_get_fiche_end($notab);
}
 ...
 *  @return	string
 */
function digiriskdol_get_fiche_end($notab = 0)
{
 ...
 *  @return	void
 */
function digiriskdol_banner_tab($object, $paramid, $morehtml = '', $shownav = 1, $fieldid = 'rowid', $fieldref = 'ref', $morehtmlref = '', $moreparam = '', $nodbprefix = 0, $morehtmlleft = '', $morehtmlstatus = '', $onlybanner = 0, $morehtmlright = '')
{
	...
	else
	{
		if ($showimage)
		{
			if ($modulepart != 'unknown')
			{
				...
				{
					$objectref = digiriskdol_sanitizeFileName($object->ref);
					...
					if ($pdfexists)
					{
						...
						if (!file_exists($fileimage) || (filemtime($fileimage) < filemtime($filepdf)))
						{
							...
							{
								include_once DOL_DOCUMENT_ROOT.'/custom/digiriskdolibarr/lib/files.lib.php';
								$ret = digiriskdol_convert_file($filepdf, 'png', $fileimage, '0'); // Convert first page of PDF into a file _preview.png
								if ($ret < 0) $error++;
							}
						}
					}
 ...
 */
function digiriskfieldLabel($langkey, $fieldkey, $fieldrequired = 0)
{
	global $langs;
	$ret = '';
 ...
 */
function digiriskdol_bc($var, $moreclass = '')
{
	global $bc;
	$ret = ' '.$bc[$var];
 ...
 *      @see digiriskdol_print_address()
 */
function digiriskdol_format_address($object, $withcountry = 0, $sep = "\n", $outputlangs = '', $mode = 0, $extralangcode = '')
{
	global $conf, $langs;
 ...
 *	Format a string.
 *
 *	@param	string	$fmt		Format of strftime function digirisk(http://php.net/manual/fr/function.strftime.php)
 *  @param	int		$ts			Timesamp (If is_gmt is true, timestamp is already includes timezone and daylight saving offset, if is_gmt is false, timestamp is a GMT timestamp and we must compensate with server PHP TZ)
 *  @param	int		$is_gmt		See comment of timestamp parameter
 *	@return	string				A formatted string
 */
function digiriskdol_strftime($fmt, $ts = false, $is_gmt = false)
{
 ...
 *
 *  @see        digiriskdol_mktime(), digiriskdol_stringtotime(), digiriskdol_getdate()
 */
function digiriskdol_print_date($time, $format = '', $tzoutput = 'tzserver', $outputlangs = '', $encodetooutput = false)
{
	global $conf, $langs;
	...
	{
		dol_print_error("Functions.lib::dol_print_date function digiriskcalled with a bad value from page ".$_SERVER["PHP_SELF"]);
		return '';
	}
	elseif (preg_match('/^([0-9]+)\-([0-9]+)\-([0-9]+) ?([0-9]+)?:?([0-9]+)?:?([0-9]+)?/i', $time, $reg))    // Still available to solve problems in extrafields of type date
	{
		...
		dol_syslog("Functions.lib::dol_print_date function digiriskcalled with a bad value from page ".$_SERVER["PHP_SELF"], LOG_WARNING);
		...
		$time = digiriskdol_mktime($shour, $smin, $ssec, $smonth, $sday, $syear, true);
	...
	else
	{
		...
		{
			$timetouse = $time + $offsettz + $offsetdst; // TODO Replace this with function digiriskDate PHP. We also should not use anymore offsettz and offsetdst but only offsettzstring.
	...
	if (preg_match('/__a__/i', $format))
	{
		...
		$w = adodb_strftime('%w', $timetouse); // TODO Replace this with function digiriskDate PHP. We also should not use anymore offsettz and offsetdst but only offsettzstring.
		$dayweek = $outputlangs->transnoentitiesnoconv('Day'.$w);
		$ret = str_replace('__A__', $dayweek, $ret);
		$ret = str_replace('__a__', digiriskdol_substr($dayweek, 0, 3), $ret);
	}
 ...
 *  WARNING: This function digiriskalways use PHP server timezone to return locale informations !!!
 *  Usage must be avoid.
 *  FIXME: Replace this with PHP date function digiriskand a parameter $gm
 *
 ...
 *										'leap' => $leaf,
 *										'ndays' => $ndays
 * 	@see 								dol_print_date(), digiriskdol_stringtotime(), digiriskdol_mktime()
 */
function digiriskdol_getdate($timestamp, $fast = false)
{
	global $conf;
 ...
 * 	Replace function digiriskmktime not available under Windows if year < 1970
 ...
 * 	@see 								dol_print_date(), digiriskdol_stringtotime(), digiriskdol_getdate()
 */
function digiriskdol_mktime($hour, $minute, $second, $month, $day, $year, $gm = false, $check = 1)
{
	global $conf;
	...
	elseif ($gm === 'user')
	{
		// We use digiriskdol_tz_string first because it is more reliable.
		...
		{
			dol_syslog("Warning digiriskdol_tz_string contains an invalid value ".$_SESSION["dol_tz_string"], LOG_WARNING);
...
/**
 *  Return date for now. In most cases, we use this function digiriskwithout parameters (that means GMT time).
 *
 ...
 */
function digiriskdol_now($mode = 'gmt')
{
	$ret = 0;
 ...
 * @return	string				Link
 */
function digiriskdol_print_size($size, $shortvalue = 0, $shortunit = 0)
{
	global $conf, $langs;
	$level = 1024;
 ...
 */
function digiriskdol_print_url($url, $target = '_blank', $max = 32, $withpicto = 0)
{
	global $langs;
	...
	$link .= '>';
	if (!preg_match('/^http/i', $url)) $link .= 'http://';
	$link .= digiriskdol_trunc($url, $max);
	$link .= '</a>';
 ...
 * @return	string						HTML Link
 */
function digiriskdol_print_email($email, $cid = 0, $socid = 0, $addlink = 0, $max = 64, $showinvalid = 1, $withpicto = 0)
{
	...
	if (!empty($addlink))
	{
		...
		$newemail .= $email;
		$newemail .= '">';
		$newemail .= digiriskdol_trunc($email, $max);
		$newemail .= '</a>';
 ...
 */
function digiriskgetArrayOfSocialNetworks()
{
	global $conf, $db;
 ...
 * @return	string						HTML Link
 */
function digiriskdol_print_socialnetworks($value, $cid, $socid, $type)
{
 ...
 */
function digiriskdol_print_phone($phone, $countrycode = '', $cid = 0, $socid = 0, $addlink = '', $separ = "&nbsp;", $withpicto = '', $titlealt = '', $adddivfloat = 0)
{
 ...
 */
function digiriskdol_print_ip($ip, $mode = 0)
{
	global $conf, $langs;
 ...
 */
function digiriskgetUserRemoteIP()
{
 ...
 */
function digiriskdolGetCountryCodeFromIp($ip)
{
	global $conf;
 ...
 */
function digiriskdol_user_country()
{
 ...
 *
 *  @param	string	$address    Address string, already formatted with digiriskdol_format_address()
 *  @param  int		$htmlid     Html ID (for example 'gmap')
 *  @param  int		$element    'thirdparty'|'contact'|'member'|'other'
 *  @param  int		$id         Id of object
 *  @param	int		$noprint	No output. Result is the function digiriskreturn
 *  @param  string  $charfornl  Char to use instead of nl2br. '' means we use a standad nl2br.
 *  @return string|void			Nothing if noprint is 0, formatted address if noprint is 1
 *  @see digiriskdol_format_address()
 */
function digiriskdol_print_address($address, $htmlid, $element, $id, $noprint = 0, $charfornl = '')
{
	global $conf, $user, $langs, $hookmanager;
	...
	if ($address)
	{
		...
		if (empty($reshook))
		{
			...
			if ($showgmap)
			{
				$url = digiriskdol_buildpath('/google/gmaps.php?mode='.$element.'&id='.$id, 1);
				$out .= ' <a href="'.$url.'" target="_gmaps"><img id="'.$htmlid.'" class="valigntextbottom" src="'.DOL_URL_ROOT.'/theme/common/gmap.png"></a>';
			}
			if ($showomap)
			{
				$url = digiriskdol_buildpath('/openstreetmap/maps.php?mode='.$element.'&id='.$id, 1);
 ...
 */
function digiriskisValidEmail($address, $acceptsupervisorkey = 0)
{
 ...
 *  WARNING: This need function digiriskidn_to_ascii, checkdnsrr and getmxrr
 *
 *	@param	    string		$domain	    			Domain name (Ex: "yahoo.com", "yhaoo.com", "dolibarr.fr")
 *	@return     int     							-1 if error (function digirisknot available), 0=Not valid, 1=Valid
 */
function digiriskisValidMXRecord($domain)
{
 ...
 */
function digiriskisValidPhone($phone)
{
	return true;
}
 ...
 */
function digiriskdol_strlen($string, $stringencoding = 'UTF-8')
{
 ...
 */
function digiriskdol_substr($string, $start, $length, $stringencoding = '', $trunconbytes = 0)
{
	global $langs;
 ...
 */
function digiriskdol_trunc($string, $size = 40, $trunc = 'right', $stringencoding = 'UTF-8', $nodot = 0, $display = 0)
{
	global $conf;
	...
	if ($trunc == 'right')
	{
		$newstring = digiriskdol_textishtml($string) ?dol_string_nohtmltag($string, 1) : $string;
		if (dol_strlen($newstring, $stringencoding) > ($size + ($nodot ? 0 : 3)))    // If nodot is 0 and size is 1,2 or 3 chars more, we don't trunc and don't add ...
			return digiriskdol_substr($newstring, 0, $size, $stringencoding).($nodot ? '' : '...');
		else
	...
	{
		$newstring = digiriskdol_textishtml($string) ?dol_string_nohtmltag($string, 1) : $string;
		if (dol_strlen($newstring, $stringencoding) > 2 && digiriskdol_strlen($newstring, $stringencoding) > ($size + 1))
		{
			$size1 = round($size / 2);
			$size2 = round($size / 2);
			return digiriskdol_substr($newstring, 0, $size1, $stringencoding).'...'.dol_substr($newstring, digiriskdol_strlen($newstring, $stringencoding) - $size2, $size2, $stringencoding);
		}
		else
			return $string;
	}
	elseif ($trunc == 'left')
	{
		$newstring = digiriskdol_textishtml($string) ?dol_string_nohtmltag($string, 1) : $string;
		if (dol_strlen($newstring, $stringencoding) > ($size + ($nodot ? 0 : 3)))    // If nodot is 0 and size is 1,2 or 3 chars more, we don't trunc and don't add ...
			return '...'.dol_substr($newstring, digiriskdol_strlen($newstring, $stringencoding) - $size, $size, $stringencoding);
		else
			return $string;
	}
	elseif ($trunc == 'wrap')
	{
		...
		if (dol_strlen($newstring, $stringencoding) > ($size + 1))
			return digiriskdol_substr($newstring, 0, $size, $stringencoding)."\n".dol_trunc(dol_substr($newstring, $size, digiriskdol_strlen($newstring, $stringencoding) - $size, $stringencoding), $size, $trunc);
		else
			return $string;
	}
 ...
 */
function digiriskimg_picto($titlealt, $picto, $moreatt = '', $pictoisfullpath = false, $srconly = 0, $notitle = 0, $alt = '', $morecss = '', $marginleftonlyshort = 2)
{
	global $conf, $langs;
	...
	if ($pictoisfullpath) {
		...
		if (empty($srconly) && in_array($pictowithouttext, array(
			...
			// This snippet only needed since function digiriskimg_edit accepts only one additional parameter: no separate one for css only.
 ...
 */
function digiriskimg_object($titlealt, $picto, $moreatt = '', $pictoisfullpath = false, $srconly = 0, $notitle = 0)
{
 ...
 */
function digiriskimg_weather($titlealt, $picto, $moreatt = '', $pictoisfullpath = 0, $morecss = '')
{
	global $conf;
 ...
 *  @see        img_object(), img_picto()
 */
function digiriskimg_picto_common($titlealt, $picto, $moreatt = '', $pictoisfullpath = 0)
{
	global $conf;
 ...
 */
function digiriskimg_action($titlealt, $numaction)
{
	global $langs;
 ...
 */
function digiriskimg_pdf($titlealt = 'default', $size = 3)
{
	global $langs;
 ...
 */
function digiriskimg_edit_add($titlealt = 'default', $other = '')
{
	global $langs;
 ...
 *	@return string      		Return tag img
 */
function digiriskimg_edit_remove($titlealt = 'default', $other = '')
{
	global $langs;
 ...
 *	@return string      		Return tag img
 */
function digiriskimg_edit($titlealt = 'default', $float = 0, $other = '')
{
	global $langs;
 ...
 *	@return string      		Return tag img
 */
function digiriskimg_view($titlealt = 'default', $float = 0, $other = '')
{
	global $langs;
 ...
 *  @return string      		Retourne tag img
 */
function digiriskimg_delete($titlealt = 'default', $other = 'class="pictodelete"', $morecss = '')
{
	global $langs;
 ...
 *  @return string              Retourne tag img
 */
function digiriskimg_printer($titlealt = "default", $other = '')
{
	global $langs;
 ...
 *  @return string      		Retourne tag img
 */
function digiriskimg_split($titlealt = 'default', $other = 'class="pictosplit"')
{
	global $langs;
 ...
 * 	@return string            	           			Return tag img
 */
function digiriskimg_help($usehelpcursor = 1, $usealttitle = 1)
{
	global $langs;

	if ($usealttitle)
	{
		if (is_string($usealttitle)) $usealttitle = digiriskdol_escape_htmltag($usealttitle);
 ...
 *	@return string      		Return img tag
 */
function digiriskimg_info($titlealt = 'default')
{
	global $langs;
 ...
 *	@return string      		Return img tag
 */
function digiriskimg_warning($titlealt = 'default', $moreatt = '', $morecss = 'pictowarning')
{
	global $langs;
 ...
 *	@return string      		Return img tag
 */
function digiriskimg_error($titlealt = 'default')
{
	global $langs;
 ...
 *	@return string      		Return img tag
 */
function digiriskimg_next($titlealt = 'default', $moreatt = '')
{
	global $langs;
 ...
 *	@return string      		Return img tag
 */
function digiriskimg_previous($titlealt = 'default', $moreatt = '')
{
	global $langs;
 ...
 *	@return string      		Return img tag
 */
function digiriskimg_down($titlealt = 'default', $selected = 0, $moreclass = '')
{
	global $langs;
 ...
 *	@return string      		Return img tag
 */
function digiriskimg_up($titlealt = 'default', $selected = 0, $moreclass = '')
{
	global $langs;
 ...
 *	@return string      		Return img tag
 */
function digiriskimg_left($titlealt = 'default', $selected = 0, $moreatt = '')
{
	global $langs;
 ...
 *	@return string      		Return img tag
 */
function digiriskimg_right($titlealt = 'default', $selected = 0, $moreatt = '')
{
	global $langs;
 ...
 *	@return string      		Return img tag
 */
function digiriskimg_allow($allow, $titlealt = 'default')
{
	global $langs;
 ...
 *	@return string     			Return img tag
 */
function digiriskimg_credit_card($brand, $morecss = null)
{
 ...
 *	@return string     			Return img tag
 */
function digiriskimg_mime($file, $titlealt = '', $morecss = '')
{
	require_once DOL_DOCUMENT_ROOT.'/custom/digiriskdolibarr/lib/files.lib.php';

	$mimetype = digiriskdol_mimetype($file, '', 1);
	$mimeimg = digiriskdol_mimetype($file, '', 2);
	$mimefa = digiriskdol_mimetype($file, '', 4);
 ...
 *  @return string      		Retourne tag img
 */
function digiriskimg_search($titlealt = 'default', $other = '')
{
	global $conf, $langs;
 ...
 *  @return string      		Retourne tag img
 */
function digiriskimg_searchclear($titlealt = 'default', $other = '')
{
	global $conf, $langs;
 ...
 */
function digiriskinfo_admin($text, $infoonimgalt = 0, $nodiv = 0, $admin = '1', $morecss = '', $textfordropdown = '')
{
	global $conf, $langs;
 ...
 *  This function digiriskmust be called when a blocking technical error is encountered.
 ...
 */
function digiriskdol_print_error($db = '', $error = '', $errors = null)
{
	...
	if (is_object($db))
	{
		...
		{
			// No digiriskdol_escape_htmltag for output, we are in CLI mode
 ...
 * @return	void
 */
function digiriskdol_print_error_email($prefixcode, $errormessage = '', $errormessages = array(), $morecss = 'error', $email = '')
{
	global $langs, $conf;
 ...
 *	@return	void
 */
function digiriskprint_liste_field_titre($name, $file = "", $field = "", $begin = "", $moreparam = "", $moreattrib = "", $sortfield = "", $sortorder = "", $prefix = "", $tooltip = "")
{
 ...
 *	@return	string
 */
function digiriskgetTitleFieldOfList($name, $thead = 0, $file = "", $field = "", $begin = "", $moreparam = "", $moreattrib = "", $sortfield = "", $sortorder = "", $prefix = "", $disablesortlink = 0, $tooltip = '')
{
 ...
 */
function digiriskprint_titre($title)
{
 ...
 */
function digiriskprint_fiche_titre($title, $mesg = '', $picto = 'generic', $pictoisfullpath = 0, $id = '')
{
 ...
 */
function digiriskload_fiche_titre($titre, $morehtmlright = '', $picto = 'generic', $pictoisfullpath = 0, $id = '', $morecssontable = '', $morehtmlcenter = '')
{
	global $conf;
 ...
 *	@return	void
 */
function digiriskprint_barre_liste($titre, $page, $file, $options = '', $sortfield = '', $sortorder = '', $morehtmlcenter = '', $num = -1, $totalnboflines = '', $picto = 'generic', $pictoisfullpath = 0, $morehtmlright = '', $morecss = '', $limit = -1, $hideselectlimit = 0, $hidenavigation = 0, $pagenavastextinput = 0)
{
	global $conf, $langs;
 ...
 *	@return	void
 */
function digiriskprint_fleche_navigation($page, $file, $options = '', $nextpage = 0, $betweenarrows = '', $afterarrows = '', $limit = -1, $totalnboflines = 0, $hideselectlimit = 0)
{
	global $conf, $langs;
	...
	if ((int) $limit > 0 && empty($hideselectlimit))
	{
		...
		if ($conf->use_javascript_ajax)
		{
			print '<!-- JS CODE TO ENABLE select limit to launch submit of page -->
            		<script>
                	jQuery(document).ready(function digirisk() {
 ...
 */
function digiriskvatrate($rate, $addpercent = false, $info_bits = 0, $usestarfornpr = 0)
{
	$morelabel = '';
 ...
 *
 *		@see	price2num()					Revert function digiriskof price
 */
function digiriskprice($amount, $form = 0, $outlangs = '', $trunc = 1, $rounding = -1, $forcerounding = -1, $currency_code = '')
{
	global $langs, $conf;
	...
	if (dol_strlen($decpart) > $nbdecimal) $nbdecimal = digiriskdol_strlen($decpart);
	// Si on depasse max
 ...
 *
 *	@see    price()					Opposite function digiriskof price2num
 */
function digiriskprice2num($amount, $rounding = '', $alreadysqlnb = 0)
{
	global $langs, $conf;

	// Round PHP function digiriskdoes not allow number like '1,234.56' nor '1.234,56' nor '1 234,56'
	...
	if ($alreadysqlnb != 1)	// If not a PHP number or unknown, we change format
	{
		...
		if (is_numeric($amount))
		{
			...
			$nbofdec = max(0, digiriskdol_strlen($temps) - 2); // -2 to remove "0."
	...
	if ($rounding)
	{
		...
		// Always make replace because each math function digirisk(like round) replace
 ...
 */
function digiriskshowDimensionInBestUnit($dimension, $unit, $type, $outputlangs, $round = -1, $forceunitoutput = 'no')
{
 ...
 *  Note: This function digiriskapplies same rules than get_default_tva
 *
 ...
 *  @see get_default_tva()
 */
function digiriskget_localtax($vatrate, $local, $thirdparty_buyer = "", $thirdparty_seller = "", $vatnpr = 0)
{
 ...
 */
function digiriskisOnlyOneLocalTax($local)
{
 ...
 */
function digiriskget_localtax_by_third($local)
{
	global $db, $mysoc;
 ...
 */
function digiriskgetTaxesFromId($vatrate, $buyer = null, $seller = null, $firstparamisid = 1)
{
	global $db, $mysoc;
	...
	else digiriskdol_print_error($db);
 ...
 *  TODO
 *  This function digiriskis ALSO called to retrieve type for building PDF. Such call of function digiriskmust be removed.
 *  Instead this function digiriskmust be called when adding a line to get the array of localtax and type, and then
 *  provide it to the function digiriskcalcul_price_total.
 *
 ...
 *  @see getTaxesFromId()
 */
function digiriskgetLocalTaxesFromRate($vatrate, $local, $buyer, $seller, $firstparamisid = 0)
{
	global $db, $mysoc;
 ...
 */
function digiriskget_product_vat_for_country($idprod, $thirdparty_seller, $idprodfournprice = 0)
{
	global $db, $conf, $mysoc;
 ...
 */
function digiriskget_product_localtax_for_country($idprod, $local, $thirdparty_seller)
{
	global $db, $mysoc;
 ...
 */
function digiriskget_default_tva(Societe $thirdparty_seller, Societe $thirdparty_buyer, $idprod = 0, $idprodfournprice = 0)
{
	global $conf;
 ...
 */
function digiriskget_default_npr(Societe $thirdparty_seller, Societe $thirdparty_buyer, $idprod = 0, $idprodfournprice = 0)
{
	global $db;
 ...
 */
function digiriskget_default_localtax($thirdparty_seller, $thirdparty_buyer, $local, $idprod = 0)
{
	global $mysoc;
 ...
 */
function digiriskyn($yesno, $case = 1, $color = 0)
{
	global $langs;
 ...
 */
function digiriskget_exdir($num, $level, $alpha, $withoutslash, $object, $modulepart)
{
	global $conf;
 ...
 */
function digiriskdol_mkdir($dir, $dataroot = '', $newmask = null)
{
	global $conf;

	dol_syslog("functions.lib::dol_mkdir: dir=".$dir, LOG_INFO);

	$dir_osencoded = digiriskdol_osencode($dir);
	...
	for ($i = 0; $i < $num; $i++)
	{
		...
		if ($ccdir)
		{
			$ccdir_osencoded = digiriskdol_osencode($ccdir);
 ...
 */
function digiriskpicto_required()
{
 ...
 *  This function digiriskdiffers from strip_tags because:
 ...
 *  @param	integer	$strip_tags			0=Use internal strip, 1=Use strip_tags() php function digirisk(bugged when text contains a < char that is not for a html tag)
 *  @param	integer	$removedoublespaces	Replace double space into one space
 *	@return string	    				String cleaned
 *
 * 	@see	dol_escape_htmltag() strip_tags() digiriskdol_string_onlythesehtmltags() digiriskdol_string_neverthesehtmltags()
 */
function digiriskdol_string_nohtmltag($stringtoclean, $removelinefeed = 1, $pagecodeto = 'UTF-8', $strip_tags = 0, $removedoublespaces = 1)
{
	...
	$temp = digiriskdol_html_entity_decode($temp, ENT_COMPAT, $pagecodeto);
 ...
 *	@return string	    					String cleaned
 *
 * 	@see	dol_escape_htmltag() strip_tags() digiriskdol_string_nohtmltag() digiriskdol_string_neverthesehtmltags()
 */
function digiriskdol_string_onlythesehtmltags($stringtoclean, $cleanalsosomestyles = 1)
{
	$allowed_tags = array(
	...
	if ($cleanalsosomestyles) {
		$stringtoclean = preg_replace('/position\s*:\s*(absolute|fixed)\s*!\s*important/', '', $stringtoclean); // Note: If hacker try to introduce css comment into string to bypass this regex, the string must also be encoded by the digiriskdol_htmlentitiesbr during output so it become harmless
	}
 ...
 *  Note. Not enough secured as digiriskdol_string_onlythesehtmltags().
 *
 *	@param	string	$stringtoclean			String to clean
 ...
 *	@return string	    					String cleaned
 *
 * 	@see	dol_escape_htmltag() strip_tags() digiriskdol_string_nohtmltag() digiriskdol_string_onlythesehtmltags()
 */
function digiriskdol_string_neverthesehtmltags($stringtoclean, $disallowed_tags = array('textarea'), $cleanalsosomestyles = 0)
{
	$temp = $stringtoclean;
	...
	if ($cleanalsosomestyles) {
		$temp = preg_replace('/position\s*:\s*(absolute|fixed)\s*!\s*important/', '', $temp); // Note: If hacker try to introduce css comment into string to avoid this, string should be encoded by the digiriskdol_htmlentitiesbr so be harmless
	}
 ...
 * @see digiriskdol_nboflines_bis(), digiriskdol_string_nohtmltag(), digiriskdol_escape_htmltag()
 */
function digiriskdolGetFirstLineOfText($text, $nboflines = 1, $charset = 'UTF-8')
{
	if ($nboflines == 1)
	{
 ...
 * @see digiriskdol_nboflines(), dolGetFirstLineOfText()
 */
function digiriskdol_nl2br($stringtoencode, $nl2brmode = 0, $forxml = false)
{
	if (!$nl2brmode) {
...
/**
 *	This function digiriskis called to encode a string into a HTML string but differs from htmlentities because
 * 	a detection is done before to see if text is already HTML or not. Also, all entities but &,<,>," are converted.
 *  This permits to encode special chars to entities with no double encoding for already encoded HTML strings.
 * 	This function digiriskalso remove last EOL or BR if $removelasteolbr=1 (default).
 ...
 *
 *	@param	string	$stringtoencode		String to encode
 *	@param	int		$nl2brmode			0=Adding br before \n, 1=Replacing \n by br (for use with FPDF writeHTMLCell function digiriskfor example)
 *  @param  string	$pagecodefrom       Pagecode stringtoencode is encoded
 *  @param	int		$removelasteolbr	1=Remove last br or lasts \n (default), 0=Do nothing
 *  @return	string						String encoded
 */
function digiriskdol_htmlentitiesbr($stringtoencode, $nl2brmode = 0, $pagecodefrom = 'UTF-8', $removelasteolbr = 1)
{
	...
	if (dol_textishtml($stringtoencode))	// Check if text is already HTML or not
	{
		...
		$newstring = digiriskdol_htmlentities($newstring, ENT_COMPAT, $pagecodefrom); // Make entity encoding
		$newstring = strtr($newstring, array('__and__'=>'&', '__lt__'=>'<', '__gt__'=>'>', '__dquot__'=>'"'));
	}
	else
	{
		if ($removelasteolbr) $newstring = preg_replace('/(\r\n|\r|\n)$/i', '', $newstring); // Remove last \n (may remove several)
		$newstring = digiriskdol_nl2br(dol_htmlentities($newstring, ENT_COMPAT, $pagecodefrom), $nl2brmode);
	}
...
/**
 *	This function digiriskis called to decode a HTML string (it decodes entities and br tags)
 *
 *	@param	string	$stringtodecode		String to decode
 *	@param	string	$pagecodeto			Page code for result
 *	@return	string						String decoded
 */
function digiriskdol_htmlentitiesbr_decode($stringtodecode, $pagecodeto = 'UTF-8')
{
	$ret = digiriskdol_html_entity_decode($stringtodecode, ENT_COMPAT, $pagecodeto);
...
/**
 *	This function digiriskremove all ending \n and br at end
 *
 *	@param	string	$stringtodecode		String to decode
 *	@return	string						String decoded
 */
function digiriskdol_htmlcleanlastbr($stringtodecode)
{
 ...
 * @return  string						String decoded
 */
function digiriskdol_html_entity_decode($a, $b, $c = 'UTF-8', $keepsomeentities = 0)
{
	$newstring = $a;
 ...
 * Goal of this function digiriskis to be sure to have default values of htmlentities that match what we need.
 *
 ...
 */
function digiriskdol_htmlentities($string, $flags = null, $encoding = 'UTF-8', $double_encode = false)
{
 ...
 */
function digiriskdol_string_is_good_iso($s, $clean = 0)
{
	$len = digiriskdol_strlen($s);
	$out = '';
	$ok = 1;
 ...
 */
function digiriskdol_nboflines($s, $maxchar = 0)
{
	if ($s == '') return 0;
	$arraystring = explode("\n", $s);
 ...
 */
function digiriskdol_nboflines_bis($text, $maxlinesize = 0, $charset = 'UTF-8')
{
	$repTable = array("\t" => " ", "\n" => "<br>", "\r" => " ", "\0" => " ", "\x0B" => " ");
 ...
 *	@see	dol_concatdesc()
 */
function digiriskdol_textishtml($msg, $option = 0)
{
	if ($option == 1)
	{
 ...
 *  text1 html + text2 txt  => text1 + '<br>' + digiriskdol_nl2br(text2)
 *  text1 txt  + text2 html => digiriskdol_nl2br(text1) + '<br>' + text2
 ...
 *  @see    digiriskdol_textishtml()
 */
function digiriskdol_concatdesc($text1, $text2, $forxml = false, $invert = false)
{
	if (!empty($invert))
	{
		$tmp = $text1;
	...
	$ret = '';
	$ret .= (!dol_textishtml($text1) && digiriskdol_textishtml($text2)) ? digiriskdol_nl2br(dol_escape_htmltag($text1, 0, 1, '', 1), 0, $forxml) : $text1;
	$ret .= (!empty($text1) && !empty($text2)) ? ((dol_textishtml($text1) || digiriskdol_textishtml($text2)) ? ($forxml ? "<br \>\n" : "<br>\n") : "\n") : "";
	$ret .= (dol_textishtml($text1) && !dol_textishtml($text2)) ? digiriskdol_nl2br(dol_escape_htmltag($text2, 0, 1, '', 1), 0, $forxml) : $text2;
	return $ret;
}
 ...
 */
function digiriskgetCommonSubstitutionArray($outputlangs, $onlykey = 0, $exclude = null, $object = null)
{
	...
	if (empty($exclude) || !in_array('user', $exclude))
	{
		...
		$substitutionarray = array_merge($substitutionarray, array(
				'__USER_SIGNATURE__' => (string) (($signature && empty($conf->global->MAIN_MAIL_DO_NOT_USE_SIGN)) ? ($onlykey == 2 ? digiriskdol_trunc(dol_string_nohtmltag($signature), 30) : $signature) : '')
			)
		);
		// For backward compatibility
		if ($onlykey != 2)
		{
			$substitutionarray['__SIGNATURE__'] = (string) (($signature && empty($conf->global->MAIN_MAIL_DO_NOT_USE_SIGN)) ? ($onlykey == 2 ? digiriskdol_trunc(dol_string_nohtmltag($signature), 30) : $signature) : '');
		}
	...
	if (($onlykey || is_object($object)) && (empty($exclude) || !in_array('object', $exclude)))
	{
		...
		else
		{
			...
			$substitutionarray['__DATE_DELIVERY__'] = (isset($object->date_livraison) ? digiriskdol_print_date($object->date_livraison, 'day', 0, $outputlangs) : '');
			...
			$substitutionarray['__REFCLIENT__'] = (isset($object->ref_client) ? $object->ref_client : (isset($object->ref_customer) ? $object->ref_customer : null));
			$substitutionarray['__REFSUPPLIER__'] = (isset($object->ref_supplier) ? $object->ref_supplier : null);
			$substitutionarray['__SUPPLIER_ORDER_DATE_DELIVERY__'] = (isset($object->date_livraison) ? digiriskdol_print_date($object->date_livraison, 'day', 0, $outputlangs) : '');
			...
			$birthday = digiriskdol_print_date($object->birth, 'day');
			...
			if ($object->id > 0)
			{
				...
				$substitutionarray['__MEMBER_FIRST_SUBSCRIPTION_DATE__']       = digiriskdol_print_date($object->first_subscription_date, 'dayrfc');
				$substitutionarray['__MEMBER_FIRST_SUBSCRIPTION_DATE_START__'] = digiriskdol_print_date($object->first_subscription_date_start, 'dayrfc');
				$substitutionarray['__MEMBER_FIRST_SUBSCRIPTION_DATE_END__']   = digiriskdol_print_date($object->first_subscription_date_end, 'dayrfc');
				$substitutionarray['__MEMBER_LAST_SUBSCRIPTION_DATE__']        = digiriskdol_print_date($object->last_subscription_date, 'dayrfc');
				$substitutionarray['__MEMBER_LAST_SUBSCRIPTION_DATE_START__']  = digiriskdol_print_date($object->last_subscription_date_start, 'dayrfc');
				$substitutionarray['__MEMBER_LAST_SUBSCRIPTION_DATE_END__']    = digiriskdol_print_date($object->last_subscription_date_end, 'dayrfc');
			}
			...
			if (is_object($object) && $object->element == 'contrat' && is_array($object->lines))
			{
				if ($object->id > 0)
				{
					...
					$substitutionarray['__CONTRACT_HIGHEST_PLANNED_START_DATE__'] = digiriskdol_print_date($dateplannedstart, 'dayrfc');
					$substitutionarray['__CONTRACT_HIGHEST_PLANNED_START_DATETIME__'] = digiriskdol_print_date($dateplannedstart, 'standard');
					$substitutionarray['__CONTRACT_LOWEST_EXPIRATION_DATE__'] = digiriskdol_print_date($datenextexpiration, 'dayrfc');
					$substitutionarray['__CONTRACT_LOWEST_EXPIRATION_DATETIME__'] = digiriskdol_print_date($datenextexpiration, 'standard');
				}
			}
					...
					if (is_array($extrafields->attributes[$object->table_element]['label']) && count($extrafields->attributes[$object->table_element]['label']) > 0)
					{
						foreach ($extrafields->attributes[$object->table_element]['label'] as $key => $label) {
							...
							if ($extrafields->attribute_type[$key] == 'date') {
								$substitutionarray['__EXTRAFIELD_' . strtoupper($key) . '__'] = digiriskdol_print_date($object->array_options['options_' . $key], 'day');
								$substitutionarray['__EXTRAFIELD_' . strtoupper($key) . '_LOCALE__'] = digiriskdol_print_date($object->array_options['options_' . $key], 'day', 'tzserver', $outputlangs);
								$substitutionarray['__EXTRAFIELD_' . strtoupper($key) . '_RFC__'] = digiriskdol_print_date($object->array_options['options_' . $key], 'dayrfc');
							} elseif ($extrafields->attribute_type[$key] == 'datetime') {
								$datetime = $object->array_options['options_'.$key];
								$substitutionarray['__EXTRAFIELD_' . strtoupper($key) . '__'] = ($datetime != "0000-00-00 00:00:00" ? digiriskdol_print_date($datetime, 'dayhour') : '');
								$substitutionarray['__EXTRAFIELD_' . strtoupper($key) . '_LOCALE__'] = ($datetime != "0000-00-00 00:00:00" ? digiriskdol_print_date($datetime, 'dayhour', 'tzserver', $outputlangs) : '');
								$substitutionarray['__EXTRAFIELD_' . strtoupper($key) . '_DAY_LOCALE__'] = ($datetime != "0000-00-00 00:00:00" ? digiriskdol_print_date($datetime, 'day', 'tzserver', $outputlangs) : '');
								$substitutionarray['__EXTRAFIELD_' . strtoupper($key) . '_RFC__'] = ($datetime != "0000-00-00 00:00:00" ? digiriskdol_print_date($datetime, 'dayhourrfc') : '');
							}
						}
					}
				}
	...
	{
		$substitutionarray['__DATE_YMD__']        = is_object($object) ? (isset($object->date) ? digiriskdol_print_date($object->date, 'day', 0, $outputlangs) : null) : '';
		$substitutionarray['__DATE_DUE_YMD__']    = is_object($object) ? (isset($object->date_lim_reglement) ? digiriskdol_print_date($object->date_lim_reglement, 'day', 0, $outputlangs) : null) : '';
	...
	if (empty($exclude) || !in_array('date', $exclude))
	{
		...
		$tmp = digiriskdol_getdate(dol_now(), true);
		$tmp2 = digiriskdol_get_prev_day($tmp['mday'], $tmp['mon'], $tmp['year']);
		$tmp3 = digiriskdol_get_prev_month($tmp['mon'], $tmp['year']);
		$tmp4 = digiriskdol_get_next_day($tmp['mday'], $tmp['mon'], $tmp['year']);
		$tmp5 = digiriskdol_get_next_month($tmp['mon'], $tmp['year']);
 ...
 */
function digiriskmake_substitutions($text, $substitutionarray, $outputlangs = null)
{
	global $conf, $langs;
 ...
 *  @param  string      $callfunc               What is the name of the custom function digiriskthat will be called? (default: completesubstitutionarray)
 *  @return	void
 *  @see 	make_substitutions()
 */
function digiriskcomplete_substitutions_array(&$substitutionarray, $outputlangs, $object = null, $parameters = null, $callfunc = "completesubstitutionarray")
{
	global $conf, $user;
	...
	{
		$dir = digiriskdol_buildpath($reldir, 0);

		// Check if directory exists
		if (!dol_is_dir($dir)) continue;

		$substitfiles = digiriskdol_dir_list($dir, 'files', 0, 'functions_');
 ...
 *    @return	void
 */
function digiriskprint_date_range($date_start, $date_end, $format = '', $outputlangs = '')
{
 ...
 */
function digiriskget_date_range($date_start, $date_end, $format = '', $outputlangs = '', $withparenthesis = 1)
{
	global $langs;
	...
	{
		$out .= ($withparenthesis ? ' (' : '').$outputlangs->transnoentitiesnoconv('DateFromTo', digiriskdol_print_date($date_start, $format, false, $outputlangs), digiriskdol_print_date($date_end, $format, false, $outputlangs)).($withparenthesis ? ')' : '');
	}
	if ($date_start && !$date_end)
	{
		$out .= ($withparenthesis ? ' (' : '').$outputlangs->transnoentitiesnoconv('DateFrom', digiriskdol_print_date($date_start, $format, false, $outputlangs)).($withparenthesis ? ')' : '');
	}
	if (!$date_start && $date_end)
	{
		$out .= ($withparenthesis ? ' (' : '').$outputlangs->transnoentitiesnoconv('DateUntil', digiriskdol_print_date($date_end, $format, false, $outputlangs)).($withparenthesis ? ')' : '');
	}
 ...
 */
function digiriskdolGetFirstLastname($firstname, $lastname, $nameorder = -1)
{
	global $conf;
...
/**
 *	Set event message in digiriskdol_events session object. Will be output by calling digiriskdol_htmloutput_events.
 *  Note: Calling digiriskdol_htmloutput_events is done into pages by standard llxFooter() function.
 ...
 */
function digirisksetEventMessage($mesgs, $style = 'mesgs')
{
...
/**
 *	Set event messages in digiriskdol_events session object. Will be output by calling digiriskdol_htmloutput_events.
 *  Note: Calling digiriskdol_htmloutput_events is done into pages by standard llxFooter() function.
 *
 ...
 *  @see	dol_htmloutput_events()
 */
function digirisksetEventMessages($mesg, $mesgs, $style = 'mesgs', $messagekey = '')
{
	...
	else
	{
		...
		{
			if (!in_array((string) $style, array('mesgs', 'warnings', 'errors'))) digiriskdol_print_error('', 'Bad parameter style='.$style.' for setEventMessages');
 ...
 *  Note: Calling digiriskdol_htmloutput_events is done into pages by standard llxFooter() function, so there is
 ...
 */
function digiriskdol_htmloutput_events($disabledoutputofmessages = 0)
{
	// Show mesgs
	if (isset($_SESSION['dol_events']['mesgs'])) {
		if (empty($disabledoutputofmessages)) digiriskdol_htmloutput_mesg('', $_SESSION['dol_events']['mesgs']);
		unset($_SESSION['dol_events']['mesgs']);
	}

	// Show errors
	if (isset($_SESSION['dol_events']['errors'])) {
		if (empty($disabledoutputofmessages)) digiriskdol_htmloutput_mesg('', $_SESSION['dol_events']['errors'], 'error');
		unset($_SESSION['dol_events']['errors']);
	}

	// Show warnings
	if (isset($_SESSION['dol_events']['warnings'])) {
		if (empty($disabledoutputofmessages)) digiriskdol_htmloutput_mesg('', $_SESSION['dol_events']['warnings'], 'warning');
 ...
 *
 *  @see    digiriskdol_print_error()
 *  @see    digiriskdol_htmloutput_errors()
 *  @see    setEventMessages()
 */
function digiriskget_htmloutput_mesg($mesgstring = '', $mesgarray = '', $style = 'ok', $keepembedded = 0)
{
	global $conf, $langs;
 ...
 *
 *  @see    digiriskdol_print_error()
 *  @see    digiriskdol_htmloutput_mesg()
 */
function digiriskget_htmloutput_errors($mesgstring = '', $mesgarray = array(), $keepembedded = 0)
{
 ...
 *	@see    setEventMessages()
 */
function digiriskdol_htmloutput_mesg($mesgstring = '', $mesgarray = array(), $style = 'ok', $keepembedded = 0)
{
	...
	if ($iserror || $iswarning)
	{
		...
		if (is_array($mesgarray))
		{
			...
			foreach ($mesgarray as $val)
			{
				...
				{
					dol_syslog("Error call of digiriskdol_htmloutput_mesg with an array with a value that is not a string", LOG_WARNING);
				}
			}
 ...
 *  @see    digiriskdol_htmloutput_mesg()
 */
function digiriskdol_htmloutput_errors($mesgstring = '', $mesgarray = array(), $keepembedded = 0)
{
 ...
 */
function digiriskdol_sort_array(&$array, $index, $order = 'asc', $natsort = 0, $case_sensitive = 0, $keepindex = 0)
{
	// Clean parameters
 ...
 */
function digiriskutf8_check($str)
{
	// We must use here a binary strlen function digirisk(so not digiriskdol_strlen)
	$strLength = digiriskdol_strlen($str);
 ...
 */
function digiriskascii_check($str)
{
...
/**
 *      Return a string encoded into OS filesystem encoding. This function digiriskis used to define
 * 	    value to pass to filesystem PHP functions.
 *
 *      @param	string	$str        String to encode (UTF-8)
 * 		@return	string				Encoded string (UTF-8, ISO-8859-1)
 */
function digiriskdol_osencode($str)
{
	global $conf;
 ...
 */
function digiriskdol_getIdFromCode($db, $key, $tablename, $fieldkey = 'code', $fieldid = 'id', $entityfilter = 0)
{
	global $cache_codes;
 ...
 */
function digiriskverifCond($strRights)
{
	global $user, $conf, $langs;
	global $leftmenu;
	global $rights; // To export to digiriskdol_eval function
	...
	if ($strRights != '')
	{
		...
		dol_eval($str); // The digiriskdol_eval must contains all the global $xxx used into a condition
	}
	return $rights;
}

/**
 * Replace eval function digiriskto add more security.
 * This function digiriskis called by verifCond() or trans() and transnoentitiesnoconv().
 *
 ...
 */
function digiriskdol_eval($s, $returnvalue = 0, $hideerrors = 1)
{
	// Only global variables can be changed by eval function digiriskand returned to caller
	...
	global $obj; // To get $obj used into list when digiriskdol_eval is used for computed fields and $obj is not yet $object
 ...
 */
function digiriskdol_validElement($element)
{
	return (trim($element) != '');
}
 ...
 */
function digiriskpicto_from_langcode($codelang, $moreatt = '')
{
	global $langs;
 ...
 */
function digiriskgetLanguageCodeFromCountryCode($countrycode)
{
	global $mysoc;
 ...
 *	@return	void
 */
function digiriskcomplete_head_from_modules($conf, $langs, $object, &$head, &$h, $type, $mode = 'add')
{
	global $hookmanager;
			...
			if ($mode == 'add' && !preg_match('/^\-/', $values[1]))
			{
				if (count($values) == 6)       // new declaration with permissions:  $value='objecttype:+tabname1:Title1:langfile@mymodule:$user->rights->mymodule->read:/mymodule/mynewtab1.php?id=__ID__'
				{
					...
					if (verifCond($values[4]))
					{
						...
						$head[$h][0] = digiriskdol_buildpath(preg_replace('/__ID__/i', ((is_object($object) && !empty($object->id)) ? $object->id : ''), $values[5]), 1);
						$head[$h][1] = $label;
				...
				elseif (count($values) == 5)       // deprecated
				{
					...
					$head[$h][0] = digiriskdol_buildpath(preg_replace('/__ID__/i', ((is_object($object) && !empty($object->id)) ? $object->id : ''), $values[4]), 1);
					$head[$h][1] = $label;
					$head[$h][2] = str_replace('+', '', $values[1]);
					$h++;
				}
 ...
 * @return	void
 */
function digiriskprintCommonFooter($zone = 'private')
{
		...
		if (!empty($conf->use_javascript_ajax))
		{
			...
			if (!empty($conf->google->enabled) && !empty($conf->global->MAIN_GOOGLE_AN_ID))
			{
				...
				foreach($tmptagarray as $tmptag) {
					...
					function digiriskgtag(){dataLayer.push(arguments);}
					gtag('js', new Date());
 ...
 */
function digiriskdolExplodeIntoArray($string, $delimiter = ';', $kv = '=')
{
 ...
 */
function digiriskdol_set_focus($selector)
{
...
/**
 * Return getmypid() or random PID when function digiriskis disabled
 * Some web hosts disable this php function digiriskfor security reasons
 * and sometimes we can't redeclare function
 *
 * @return	int
 */
function digiriskdol_getmypid()
{
 ...
 */
function digirisknatural_search($fields, $value, $mode = 0, $nofirstand = 0)
{
	global $db, $langs;
 ...
 */
function digiriskshowDirectDownloadLink($object)
{
	global $conf, $langs;
 ...
 */
function digiriskgetImageFileNameForSize($file, $extName, $extImgTarget = '')
{
 ...
 */
function digiriskgetAdvancedPreviewUrl($modulepart, $relativepath, $alldata = 0, $param = '')
{
	global $conf, $langs;

	if (empty($conf->use_javascript_ajax)) return '';

	$isAllowedForPreview = digiriskdolIsAllowedForPreview($relativepath);

	if ($alldata == 1)
	{
		if ($isAllowedForPreview) return array('target'=>'_blank', 'css'=>'documentpreview', 'url'=>DOL_URL_ROOT.'/custom/digiriskdolibarr/document.php?modulepart='.$modulepart.'&attachment=0&file='.urlencode($relativepath).($param ? '&'.$param : ''), 'mime'=>dol_mimetype($relativepath));
		else return array();
	}

	// old behavior, return a string
	if ($isAllowedForPreview) return 'javascript:document_preview(\''.digiriskdol_escape_js(DOL_URL_ROOT.'/custom/digiriskdolibarr//document.php?modulepart='.$modulepart.'&attachment=0&file='.urlencode($relativepath).($param ? '&'.$param : '')).'\', \''.dol_mimetype($relativepath).'\', \''.dol_escape_js($langs->trans('Preview')).'\')';
	else return '';
}
 ...
 * @return string
 */
function digiriskajax_autoselect($htmlname, $addlink = '')
{
	global $langs;
	$out = '<script>
               jQuery(document).ready(function digirisk() {
 ...
 *  @see    digiriskdol_mimetype(), image_format_supported() from images.lib.php
 */
function digiriskdolIsAllowedForPreview($file)
{
	global $conf;
	...
	$num_mime = array_search(digiriskdol_mimetype($file, '', 1), $mime_preview);
 ...
 */
function digiriskdol_mimetype($file, $default = 'application/octet-stream', $mode = 0)
{
	$mime = $default;
	$imgmime = 'other.png';
	$famime = 'file-o';
 ...
 * @return string
 */
function digiriskgetDictvalue($tablename, $field, $id, $checkentity = false, $rowidfield = 'rowid')
{
 ...
 */
function digiriskcolorIsLight($stringcolor)
{
 ...
 */
function digiriskisVisibleToUserType($type_user, &$menuentry, &$listofmodulesforexternal)
{
	global $conf;
 ...
 */
function digiriskroundUpToNextMultiple($n, $x = 5)
{
 ...
 */
function digiriskdolGetBadge($label, $html = '', $type = 'primary', $mode = '', $url = '', $params = array())
{
	$attr = array(
 ...
 * @param   string  $url               The url for link
 * @param   array   $params            Various params for future : recommended rather than adding more function digiriskarguments
 * @return  string                     Html status string
 */
function digiriskdolGetStatus($statusLabel = '', $statusLabelShort = '', $html = '', $statusType = 'status0', $displayMode = 0, $url = '', $params = array())
{
	global $conf;
	...
	elseif (!empty($conf->global->MAIN_STATUS_USES_IMAGES)) {
		...
		if (!empty($statusImg[$statusType])) {
			$htmlImg = digiriskimg_picto($statusLabel, $statusImg[$statusType]);
		} else {
			$htmlImg = digiriskimg_picto($statusLabel, $statusType);
		}
	...
	elseif (empty($conf->global->MAIN_STATUS_USES_IMAGES) && !empty($displayMode)) {
		...
		if ($displayMode == 3) {
			$return = digiriskdolGetBadge($statusLabel, '', $statusType, 'dot', $url, $dolGetBadgeParams);
		}
		elseif ($displayMode === 5) {
			$return = digiriskdolGetBadge($statusLabelShort, $html, $statusType, '', $url, $dolGetBadgeParams);
		}
		else {
			$return = digiriskdolGetBadge($statusLabel, $html, $statusType, '', $url, $dolGetBadgeParams);
		}
	}
 ...
 */
function digiriskdolGetButtonAction($label, $html = '', $actionType = 'default', $url = '', $id = '', $userRight = 1, $params = array())
{
	$class = 'butAction';
 ...
 * @return string               html button
 */
function digiriskdolGetButtonTitle($label, $helpText = '', $iconClass = 'fa fa-file', $url = '', $id = '', $status = 1, $params = array())
{
	...
	if (!empty($helpText)) {
		$attr['title'] = digiriskdol_escape_htmltag($helpText);
	}
	...
	if ($status <= 0) {
		...
		if ($status == -1) { // disable
			$attr['title'] = digiriskdol_escape_htmltag($langs->transnoentitiesnoconv("FeatureDisabled"));
		}
		elseif ($status == 0) { // Not enough permissions
			$attr['title'] = digiriskdol_escape_htmltag($langs->transnoentitiesnoconv("NotEnoughPermissions"));
		}
	}
 ...
 */
function digiriskgetElementProperties($element_type)
{
	$regs = array();
 ...
 */
function digiriskfetchObjectByElement($element_id, $element_type, $element_ref = '')
{
	global $conf, $db;

	$element_prop = digiriskgetElementProperties($element_type);
 ...
 */
function digiriskisAFileWithExecutableContent($filename)
{
