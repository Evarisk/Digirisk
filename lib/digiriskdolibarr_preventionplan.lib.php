...
function preventionplanPrepareHead($object)
{
	...
	if (isset($object->fields['note_public']) || isset($object->fields['note_private']))
	{
		...
		$head[$h][0] = dol_buildpath('/digiriskdolibarr/preventionplan_note.php', 1).'?id='.$object->id;
	...
	$upload_dir = $conf->digiriskdolibarr->dir_output."/preventionplan/".dol_sanitizeFileName($object->ref);
	$nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
	$nbLinks = Link::count($db, $object->element, $object->id);
	$head[$h][0] = dol_buildpath("/digiriskdolibarr/preventionplan_document.php", 1).'?id='.$object->id;
	...
	complete_head_from_modules($conf, $langs, $object, $head, $h, 'preventionplan@digiriskdolibarr', 'remove');
