...
class doc_listingrisksphoto_odt extends ModeleODTListingRisksPhoto
{
	...
	public function write_file(SaturneDocuments $objectDocument, Translate $outputLangs, string $srcTemplatePath, int $hideDetails = 0, int $hideDesc = 0, int $hideRef = 0, array $moreParam): int
	{
		...
		$numberingModules = [
			'digiriskdolibarrdocuments/listingrisksphoto' => $conf->global->DIGIRISKDOLIBARR_LISTINGRISKSPHOTO_ADDON
		];
		...
		$id          = $objectDocument->create($user, true, $digiriskelement);
		...
		if (file_exists($dir)) {
			...
			$hookmanager->executeHooks('ODTSubstitution', $parameters, $this, $action); // Note that $action and $objectDocument may have been modified by some hooks
			$tmparray['reference'] = $objectDocument->ref;
