...
/**
 *	\file       htdocs/core/modules/digiriskdolibarr/digiriskdocuments/actionplan/doc_actionplan_odt.modules.php
...
dol_include_once('/custom/digiriskdolibarr/core/modules/digiriskdolibarr/digiriskdocuments/actionplan/mod_actionplan_standard.php');
dol_include_once('/custom/digiriskdolibarr/core/modules/digiriskdolibarr/digiriskdocuments/actionplan/modules_actionplan.php');
/**
 *	Class to build documents using ODF templates generator
 */
class doc_actionplan_odt extends ModeleODTWorkUnitDocument
{
	/**
	 * Issuer
	 * @var Societe
	...
	public function __construct($db)
	{
		...
		$this->scandir = 'DIGIRISKDOLIBARR_ACTIONPLAN_ADDON_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function info($langs)
	{
		...
		$texte .= '<input type="hidden" name="param1" value="DIGIRISKDOLIBARR_ACTIONPLAN_ADDON_ODT_PATH">';
		$texte .= '<table class="nobordernopadding" width="100%">';

		// List of directories area
		$texte .= '<tr><td>';
		$texttitle = $langs->trans("ListOfDirectories");
		$listofdir = explode(',', preg_replace('/[\r\n]+/', ',', trim($conf->global->DIGIRISKDOLIBARR_ACTIONPLAN_ADDON_ODT_PATH)));
		$listoffiles = array();
		...
		if (!empty($conf->global->DIGIRISKDOLIBARR_ACTIONPLAN_ADDON_ODT_PATH))
		{
	...
	public function write_file($object, $outputlangs, $srctemplatepath, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $digiriskelement)
	{
		...
		{
			dol_syslog("doc_actionplan_odt::write_file parameter srctemplatepath empty", LOG_WARNING);
			return -1;
		}
		...
		$mod = new $conf->global->DIGIRISKDOLIBARR_ACTIONPLAN_ADDON($this->db);
		...
		$dir = $conf->digiriskdolibarr->multidir_output[isset($object->entity) ? $object->entity : 1] . '/actionplan';
		...
		if (file_exists($dir))
		{
			$filename = preg_split('/actionplan\//' , $srctemplatepath);
