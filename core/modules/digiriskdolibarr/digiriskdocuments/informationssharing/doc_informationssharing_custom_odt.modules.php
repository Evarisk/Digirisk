...
/**
 *	\file       core/modules/digiriskdolibarr/digiriskdocuments/informationssharing/doc_informationssharing_custom_odt.modules.php
 ...
 */
class doc_informationssharing_custom_odt extends ModeleODTInformationsSharing
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db          = $db;
		$this->name        = $langs->trans('InformationsSharingCustomDigiriskTemplate');
	...
	public function info($langs)
	{
		...
		$texte .= '<input type="hidden" name="value1" value="' . $conf->global->DIGIRISKDOLIBARR_INFORMATIONSSHARING_CUSTOM_ADDON_ODT_PATH . '">';
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' &nbsp; <a class="reposition" href="'.DOL_URL_ROOT.'/document.php?modulepart=ecm&file=digiriskdolibarr/informationssharing/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=ecm&keyforuploaddir=DIGIRISKDOLIBARR_INFORMATIONSSHARING_CUSTOM_ADDON_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picto('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
		...
		$texte .= $langs->trans("PleaseNameTheFile", 'template_informationssharing_custom.odt');
		$texte .= '</td>';
		$texte .= '</tr>';
	...
	public function write_file($object, $outputlangs, $srctemplatepath)
	{
		...
		if (empty($srctemplatepath)) {
			dol_syslog("doc_informationssharing_custom_odt::write_file parameter srctemplatepath empty", LOG_WARNING);
			return -1;
		}
