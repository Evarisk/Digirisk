...
/**
 *	\file       core/modules/digiriskdolibarr/digiriskdocuments/firepermitdocument/doc_firepermitdocument_custom_odt.modules.php
 ...
 */
class doc_firepermitdocument_custom_odt extends ModeleODTFirePermitDocument
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db          = $db;
		$this->name        = $langs->trans('FirePermitDocumentCustomDigiriskTemplate');
		$this->description = $langs->trans("DocumentModelOdt");
		$this->scandir     = 'DIGIRISKDOLIBARR_FIREPERMITDOCUMENT_CUSTOM_ADDON_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function info($langs)
	{
		...
		$texte .= '<input type="hidden" name="param1" value="DIGIRISKDOLIBARR_FIREPERMITDOCUMENT_CUSTOM_ADDON_ODT_PATH">';
		$texte .= '<input type="hidden" name="value1" value="' . $conf->global->DIGIRISKDOLIBARR_FIREPERMITDOCUMENT_CUSTOM_ADDON_ODT_PATH . '">';
		$texte .= '<table class="nobordernopadding">';

		// List of directories area
		$texte      .= '<tr><td>';
		$texttitle   = $langs->trans("ListOfDirectories");
		$listofdir   = explode(',', preg_replace('/[\r\n]+/', ',', trim($conf->global->DIGIRISKDOLIBARR_FIREPERMITDOCUMENT_CUSTOM_ADDON_ODT_PATH)));
		$listoffiles = array();
		...
		$texte .= $conf->global->DIGIRISKDOLIBARR_FIREPERMITDOCUMENT_CUSTOM_ADDON_ODT_PATH;
		$texte .= '</span>';
		...
		if ( ! empty($conf->global->DIGIRISKDOLIBARR_FIREPERMITDOCUMENT_CUSTOM_ADDON_ODT_PATH)) {
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' &nbsp; <a class="reposition" href="'.DOL_URL_ROOT.'/document.php?modulepart=ecm&file=digiriskdolibarr/firepermitdocument/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=ecm&keyforuploaddir=DIGIRISKDOLIBARR_FIREPERMITDOCUMENT_CUSTOM_ADDON_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picto('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
		...
		$texte .= '<input type="hidden" value="DIGIRISKDOLIBARR_FIREPERMITDOCUMENT_CUSTOM_ADDON_ODT_PATH" name="keyforuploaddir">';
		$texte .= '<input type="submit" class="button" value="' . dol_escape_htmltag($langs->trans("Upload")) . '" name="upload">';
		$texte .= '</div>';
		$texte .= '</td>';

		$texte .= '<td rowspan="2" class="tdtop hideonsmartphone">';
		$texte .= $langs->trans("PleaseNameTheFile", 'template_firepermitdocument_custom.odt');
		$texte .= '</td>';
		$texte .= '</tr>';
	...
	public function write_file($object, $outputlangs, $srctemplatepath)
	{
		...
		if (empty($srctemplatepath)) {
			dol_syslog("doc_firepermitdocument_custom_odt::write_file parameter srctemplatepath empty", LOG_WARNING);
			return -1;
		}
		...
		$dir                                             = $conf->digiriskdolibarr->multidir_output[isset($object->entity) ? $object->entity : 1] . '/firepermitdocument';
