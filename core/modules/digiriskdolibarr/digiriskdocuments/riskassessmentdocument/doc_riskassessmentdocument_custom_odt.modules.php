...
/**
 *	\file       core/modules/digiriskdolibarr/digiriskdocuments/riskassessmentdocument/doc_riskassessmentdocument_custom_odt.modules.php
 ...
 */
class doc_riskassessmentdocument_custom_odt extends ModeleODTRiskAssessmentDocument
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db          = $db;
		$this->name        = $langs->trans('RiskAssessmentDocumentCustomDigiriskTemplate');
		$this->description = $langs->trans("DocumentModelOdt");
		$this->scandir     = 'DIGIRISKDOLIBARR_RISKASSESSMENTDOCUMENT_CUSTOM_ADDON_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function info($langs)
	{
		...
		if ( ! empty($conf->global->DIGIRISKDOLIBARR_RISKASSESSMENTDOCUMENT_CUSTOM_ADDON_ODT_PATH)) {
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' &nbsp; <a class="reposition" href="'.DOL_URL_ROOT.'/document.php?modulepart=ecm&file=digiriskdolibarr/riskassessmentdocument/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=ecm&keyforuploaddir=DIGIRISKDOLIBARR_RISKASSESSMENTDOCUMENT_CUSTOM_ADDON_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picto('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
		...
		$texte .= '<input type="hidden" value="DIGIRISKDOLIBARR_RISKASSESSMENTDOCUMENT_CUSTOM_ADDON_ODT_PATH" name="keyforuploaddir">';
		$texte .= '<input type="submit" class="button" value="' . dol_escape_htmltag($langs->trans("Upload")) . '" name="upload">';
		$texte .= '</div>';
		$texte .= '</td>';

		$texte .= '<td rowspan="2" class="tdtop hideonsmartphone">';
		$texte .= $langs->trans("PleaseNameTheFile", 'template_riskassessmentdocument_custom.odt');
		$texte .= '</td>';
		$texte .= '</tr>';
	...
	public function write_file($object, $outputlangs, $srctemplatepath)
	{
		...
		if (empty($srctemplatepath)) {
			dol_syslog("doc_riskassessmentdocument_custom_odt::write_file parameter srctemplatepath empty", LOG_WARNING);
			return -1;
		}
