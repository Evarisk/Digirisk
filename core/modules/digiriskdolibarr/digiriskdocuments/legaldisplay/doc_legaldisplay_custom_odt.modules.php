...
/**
 *	\file       core/modules/digiriskdolibarr/digiriskdocuments/legaldisplay/doc_legaldisplay_custom_odt.modules.php
...
require_once __DIR__ . '/mod_legaldisplay_standard.php';
require_once __DIR__ . '/modules_legaldisplay.php';

/**
 *	Class to build documents using ODF templates generator
 */
class doc_legaldisplay_custom_odt extends ModeleODTLegalDisplay
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db          = $db;
		$this->name        = $langs->trans('LegalDisplayCustomDigiriskTemplate');
	...
	public function info($langs)
	{
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' &nbsp; <a class="reposition" href="'.DOL_URL_ROOT.'/document.php?modulepart=ecm&file=digiriskdolibarr/legaldisplay/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=ecm&keyforuploaddir=DIGIRISKDOLIBARR_LEGALDISPLAY_CUSTOM_ADDON_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picto('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
		...
		$texte .= $langs->trans("PleaseNameTheFile", 'template_legaldisplay_custom.odt');
		$texte .= '</td>';
		$texte .= '</tr>';
	...
	public function write_file($object, $outputlangs, $srctemplatepath)
	{
		...
		if (empty($srctemplatepath)) {
			dol_syslog("doc_legaldisplay_custom_odt::write_file parameter srctemplatepath empty", LOG_WARNING);
			return -1;
		}
