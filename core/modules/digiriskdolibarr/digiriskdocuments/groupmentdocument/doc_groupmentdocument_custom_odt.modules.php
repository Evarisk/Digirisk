...
/**
 *	\file       core/modules/digiriskdolibarr/digiriskdocuments/groupmentdocument/doc_groupmentdocument_custom_odt.modules.php
 ...
 */
class doc_groupmentdocument_custom_odt extends ModeleODTGroupmentDocument
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db          = $db;
		$this->name        = $langs->trans('GroupmentDocumentCustomDigiriskTemplate');
		$this->description = $langs->trans("DocumentModelOdt");
		$this->scandir     = 'DIGIRISKDOLIBARR_GROUPMENTDOCUMENT_CUSTOM_ADDON_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function write_file($object, $outputlangs, $srctemplatepath)
	{
		...
		if (empty($srctemplatepath)) {
			dol_syslog("doc_groupmentdocument_custom_odt::write_file parameter srctemplatepath empty", LOG_WARNING);
			return -1;
		}
		...
		if (file_exists($dir)) {
			...
			try {
				...
				if ($foundtagforlines) {
					...
					if ( ! empty($object) ) {
						$risks = $risk->fetchRisksOrderedByCotation($object->id, false, $conf->global->DIGIRISKDOLIBARR_SHOW_PARENT_RISKS, $conf->global->DIGIRISKDOLIBARR_SHOW_SHARED_RISKS);
						if ($risks < 1) {
							$risks = array();
						}
						if (! empty($risks)) {
					...
					if ( ! empty($object) ) {
						...
						if ($signalisations !== -1) {
							...
							foreach ($signalisations as $line) {
								...
								$tmparray['recommandationIcon']        = $path . '/' . $signalisation->get_risksign_category($signalisation);
