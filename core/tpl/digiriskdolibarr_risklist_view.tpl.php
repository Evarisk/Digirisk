...
<!--	RISK ASSESSMENT-->
	...
	print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $num, '', 0, $newcardbutton, '', $limit, 0, 0, 1);
	...
	while ($i < ($limit ? min($num, $limit) : $num))
	{
		...
		foreach ($evaluation->fields as $key => $val)
		{
			...
			if (!empty($arrayfields['evaluation.'.$key]['checked']))
			{
				...
				if ($key == 'cotation') {
					require_once './core/tpl/digiriskdolibarr_riskassessment_view.tpl.php';
				}
				elseif ($key == 'has_tasks' && $conf->global->DIGIRISKDOLIBARR_TASK_MANAGEMENT) {
					require_once './core/tpl/digiriskdolibarr_task_view.tpl.php';
				}
