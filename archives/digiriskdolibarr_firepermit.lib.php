 ...
 *
 * @param	FirePermit	$object		FirePermit
...
function firepermitPrepareHead($object)
{
	...
	if (isset($object->fields['note_public']) || isset($object->fields['note_private']))
	{
		...
		$head[$h][0] = dol_buildpath('/digiriskdolibarr/firepermit_note.php', 1).'?id='.$object->id;
	...
	$upload_dir = $conf->digiriskdolibarr->dir_output."/firepermit/".dol_sanitizeFileName($object->ref);
	$nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
	$nbLinks = Link::count($db, $object->element, $object->id);
	$head[$h][0] = dol_buildpath("/digiriskdolibarr/firepermit_document.php", 1).'?id='.$object->id;
	...
	//	'entity:-tabname:Title:@digiriskdolibarr:/digiriskdolibarr/mypage.php?id=__ID__'
	//); // to remove a tab
	complete_head_from_modules($conf, $langs, $object, $head, $h, 'firepermit@digiriskdolibarr');

	complete_head_from_modules($conf, $langs, $object, $head, $h, 'firepermit@digiriskdolibarr', 'remove');
