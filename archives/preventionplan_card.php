...
dol_include_once('/digiriskdolibarr/core/modules/digiriskdolibarr/mod_preventionplan_jupiter.php');
dol_include_once('/digiriskdolibarr/core/modules/digiriskdolibarr/mod_preventionplan_standard.php');
dol_include_once('/digiriskdolibarr/core/modules/digiriskdolibarr/mod_preventionplan_advanced.php');
dol_include_once('/digiriskdolibarr/core/modules/digiriskdolibarr/mod_preventionplan_titan.php');
...
$permissiontodelete = $user->rights->digiriskdolibarr->preventionplan->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
$permissionnote = $user->rights->digiriskdolibarr->preventionplan->write;
$permissiondellink = $user->rights->digiriskdolibarr->preventionplan->write;
$upload_dir = $conf->digiriskdolibarr->multidir_output[isset($object->entity) ? $object->entity : 1];
...
if (empty($reshook))
{
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			...
			else $backtopage = dol_buildpath('/digiriskdolibarr/preventionplan_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
		}
	}
	...
	{
		$object->setValueFrom('fk_soc', GETPOST('fk_soc', 'int'), '', '', 'date', '', $user, 'PREVENTIONPLAN_MODIFY');
	}
	...
	$autocopy = 'MAIN_MAIL_AUTOCOPY_PREVENTIONPLAN_TO';
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewPreventionPlan"), '', 'object_'.$object->picto);
	...
	$digirisk_addon = $conf->global->DIGIRISKDOLIBARR_PREVENTIONPLAN_ADDON;
	$modele = new $digirisk_addon($db);

	print '<tr><td class="fieldrequired">'.$langs->trans("Ref").'</td><td>';
	print '<input hidden class="flat" type="text" size="36" name="ref" value="'.$modele->prefixpreventionplan.'">';
	print $modele->prefixpreventionplan;
	print '</td></tr>';
...
{
	print load_fiche_titre($langs->trans("PreventionPlan"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	dol_fiche_head($head, 'card', $langs->trans("PreventionPlan"), -1, $object->picto);
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/digiriskdolibarr/preventionplan_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	if ($action != 'presend')
	{
		...
		if ($includedocgeneration) {
			...
			$genallowed = $user->rights->digiriskdolibarr->preventionplan->read;	// If you can read, you can build the PDF to read content
			$delallowed = $user->rights->digiriskdolibarr->preventionplan->create;	// If you can create/edit, you can remove a file on card

			print $formfile->showdocuments('digiriskdolibarr:PreventionPlan',$dir_files, $filedir, $urlsource, $genallowed, $delallowed, $conf->global->DIGIRISKDOLIBARR_PREVENTIONPLAN_DEFAULT_MODEL, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);

		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('preventionplan'));
	...
	$object->modelpdf = 'preventionplan_A4_odt';

	//get title and content
	$modelmail = 'Digirisk_PreventionPlan';
	$defaulttopic = $langs->trans('SendPreventionPlan') . ' - ' . $conf->global->MAIN_INFO_SOCIETE_NOM;
	$diroutput = $conf->digiriskdolibarr->dir_output . '/preventionplan';
	$trackid = 'preventionplan'.$object->id;
