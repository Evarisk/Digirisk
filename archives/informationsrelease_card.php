...
$hookmanager->initHooks(array('informationsrelease'));
...
// Add action
if ($action == 'add' && $user->rights->digiriskdolibarr->creer)
{
...
$helpurl = '';
llxHeader("", $langs->trans("Diffusion d'informations"), $helpurl);
...
if ($user->rights->categorie->creer)
{
	...
	if ($action == 'create' || $_POST["addcat"] == 'addcat')
	{
		...
		print load_fiche_titre($langs->trans("Diffusion d'informations"));
		...
		// Ref
		print '<tr>';
		print '<td class="titlefieldcreate fieldrequired">'.$langs->trans("Date d'élection du CSE").'</td><td><input id="label" class="minwidth100" name="label" value="'.$label.'">';
		print'</td></tr>';

		// Description 
		/* print '<tr><td class="tdtop">'.$langs->trans("Date d'élection").'</td><td>';
		...
		// Color
		print '<tr><td>'.$langs->trans("Membres du CSE").'</td><td>';
...
*/
		...
		print '<input type="submit" class="button" value="'.$langs->trans("Générer la diffusion d'informations").'" name="creation" />';
		print '&nbsp; &nbsp; &nbsp;';
