...
/**
 *   	\file       digiriskelement_list.php
 *		\ingroup    digiriskdolibarr
 *		\brief      List page for digiriskelement
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'digiriskelementlist'; // To manage different context of search
...
$hookmanager->initHooks(array('digiriskelementlist')); // Note that conf->hooks_modules contains array
...
if (empty($reshook))
{
	...
	// Mass actions
	$objectclass = 'DigiriskElement';
	$objectlabel = 'DigiriskElement';
...
//$help_url="EN:Module_DigiriskElement|FR:Module_DigiriskElement_FR|ES:Módulo_DigiriskElement";
$help_url = '';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("DigiriskElements"));
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page)
{
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/digiriskdolibarr/digiriskelement_card.php', 1).'?id='.$id);
	exit;
}
...
// Title
$title = 'Digirisk - Dolibarr '.DOL_VERSION;
if (!empty($conf->global->MAIN_APPLICATION_TITLE)) $title = 'Digirisk - '.$conf->global->MAIN_APPLICATION_TITLE;
$head = '<meta name="apple-mobile-web-app-title" content="Digirisk"/>
<meta name="apple-mobile-web-app-capable" content="yes">
<meta name="mobile-web-app-capable" content="yes">
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no"/>';

top_htmlhead($head, $title, $disablejs, $disablehead, $arrayofjs, $arrayofcss); ?>
<link rel="stylesheet" href="css/digiriskdolibarr.css.php">

<body class="bodytakepos" style="overflow: hidden;">
<?php
print '<div class="hidden dialog-info-takepos-terminal" id="dialog-info" title="Digirisk">'.$langs->trans('Digirisk').'</div>';
...
<div class="digirisk-wrap wpeo-wrap">
	<div class="navigation-container">
		...
		<div class="add-container">
			<div class="wpeo-button button-square-50 wpeo-tooltip-event" data-direction="bottom" data-color="light" aria-label="<?php echo $langs->trans('NewGroupment'); ?>" data-type="Group_Class"><span class="button-icon fas fa-home"></span><span class="button-add animated fas fa-plus-circle"></span></div>
			<div class="wpeo-button button-square-50 wpeo-tooltip-event" data-direction="bottom" data-color="light" aria-label="<?php echo $langs->trans('NewWorkUnit'); ?>" data-type="Workunit_Class"><span class="button-icon fas fa-home"></span><span class="button-add animated fas fa-plus-circle"></span></div>
		</div>
					...
					<li class="unit <?php //echo ( $society->ID === $selected_society_id ) ? 'active' : ''; echo ( \eoxia\Post_Util::is_parent( $society->ID, $selected_society_id ) ) ? 'toggled' : ''; ?>"
						...
						<div class="unit-container">
							...
							<?php if ( $element->element_type == 'groupment' ) { ?>
								<div class="add-container">
									<div class="wpeo-button button-square-50 wpeo-tooltip-event" data-direction="bottom" data-color="light" aria-label="<?php echo $langs->trans('NewGroupment'); ?>" data-type="Group_Class"><span class="button-icon dashicons dashicons-admin-multisite"></span><span class="button-add animated fas fa-plus-circle"></span></div>
									<div class="wpeo-button button-square-50 wpeo-tooltip-event" data-direction="bottom" data-color="light" aria-label="<?php echo $langs->trans('NewWorkunit'); ?>" data-type="Workunit_Class"><span class="button-icon dashicons dashicons-admin-home"></span><span class="button-add animated fas fa-plus-circle"></span></div>
								</div>
								<div class="mobile-add-container wpeo-dropdown dropdown-right option">
									<div class="dropdown-toggle"><i class="action fas fa-ellipsis-v"></i></div>
									<ul class="dropdown-content">
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/digiriskdolibarr/digiriskelement_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendDigiriskElementRef";
$modelmail = "digiriskelement";
$objecttmp = new DigiriskElement($db);
