...
if ((empty($action) || ($action != 'create' && $action != 'edit'))) {
	...
	$url = dol_buildpath('/custom/digiriskdolibarr/accident_attendants.php?id='.$object->id, 3);
	print '<a href="'.$url.'">'.$attendants.'</a>';
	print '<a class="'. ($object->status == 1 ? 'butAction' : 'butActionRefused classfortooltip').'" id="actionButtonAddAttendants" title="'.dol_escape_htmltag($langs->trans("AccidentMustBeInProgress")).'" href="'.$url.'">'.$langs->trans('AddAttendants').'</a>';
	print '</td></tr>';
