...
/**
 *  \file       	htdocs/informationsrelease/card.php
 *  \ingroup    	informationsrelease
 *  \brief      	Page for trip and expense report card
 */

require '../../main.inc.php';
dol_mkdir(DOL_DATA_ROOT.'/informationsrelease');
...
require_once DOL_DOCUMENT_ROOT.'/custom/DigiriskDolibarr/core/modules/digiriskdolibarr/mod_digiriskdolibarr_standard.php';
require_once DOL_DOCUMENT_ROOT.'/custom/DigiriskDolibarr/class/informationsrelease.class.php';
...
//$result = restrictedArea($user, 'informationsrelease', $id, 'informationsrelease');


// Hack to use informationsrelease dir
...
$conf->informationsrelease = new stdClass;
$conf->informationsrelease->dir_output = $rootfordata.'/expensereport/.pdf';
...
$object = new InformationsRelease($db);
...
$hookmanager->initHooks(array('informationsreleasecard', 'globalcard'));

$permissionnote = $user->rights->informationsrelease->creer; // Used by the include of actions_setnotes.inc.php
$permissiondellink = $user->rights->informationsrelease->creer; // Used by the include of actions_dellink.inc.php
$permissiontoadd = $user->rights->informationsrelease->creer; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php


$upload_dir = $conf->informationsrelease->dir_output.'/'.dol_sanitizeFileName($object->ref);

$filename = dol_sanitizeFileName($object->ref);
		$filedir = $conf->informationsrelease->dir_output."/".dol_sanitizeFileName($object->ref);
		$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;
		$genallowed	= $user->rights->informationsrelease->creer;
		$delallowed	= $user->rights->informationsrelease->creer;
		$var = true;
...
if ($object->id > 0)
{
    ...
    $canread = 0;
    if (!empty($user->rights->informationsrelease->readall)) $canread = 1;
    if (!empty($user->rights->informationsrelease->lire) && in_array($object->fk_user_author, $childids)) $canread = 1;
    if (!$canread)
    {
        accessforbidden();
    }
...
if (empty($reshook))
{
    ...
    // Action clone object
    if ($action == 'confirm_clone' && $confirm == 'yes' && $user->rights->informationsrelease->creer)
    {
    ...
    if ($action == 'confirm_delete' && GETPOST("confirm", 'alpha') == "yes" && $id > 0 && $user->rights->informationsrelease->supprimer)
    {
    	$object = new InformationsRelease($db);
    ...
    if ($action == 'add' && $user->rights->informationsrelease->creer)
    {
    	$object = new InformationsRelease($db);
    	...
    	if (!$error && empty($conf->global->INFORMATIONSRELASE_ALLOW_OVERLAPPING_PERIODS) && $object->periode_existe($fuser, $object->date_debut, $object->date_fin))
    	{
    		$error++;
    ...
    if ($action == 'update' && $user->rights->informationsrelease->creer)
    {
    	$object = new InformationsRelease($db);
    	$object->fetch($id);
    ...
    if ($action == 'update_extras')
    {
        ...
        if (!$error)
        {
            // Actions on extra fields
           	$result = $object->insertExtraFields('INFORMATIONSRELASE_MODIFY');
			if ($result < 0)
			{
				setEventMessages($object->error, $object->errors, 'errors');
				$error++;
    ...
    if ($action == "confirm_validate" && GETPOST("confirm", 'alpha') == "yes" && $id > 0 && $user->rights->informationsrelease->creer)
    {
    	$error = 0;
    	...
    	if (!$error && $result > 0 && $object->fk_user_validator > 0)
    	{
    		...
    		if ($emailTo && $emailFrom)
    		{
    			...
    			$subject = $societeName." - ".$langs->transnoentities("InformationsReleaseWaitingForApproval");

    			// CONTENT
    			$link = $urlwithroot.'/informationsrelease/card.php?id='.$object->id;
				$link = '<a href="'.$link.'">'.$link.'</a>';
    			$message = $langs->transnoentities("InformationsReleaseWaitingForApprovalMessage", $expediteur->getFullName($langs), get_date_range($object->date_debut, $object->date_fin, '', $langs), $link);

    			// Rebuild pdf
    			/*
    			$object->setDocModel($user,"");
    			$resultPDF = informationsrelease_pdf_create($db,$id,'',"",$langs);

    			if($resultPDF):
    			// ATTACHMENT
    			array_push($filename,dol_sanitizeFileName($object->ref).".pdf");
    			array_push($filedir,$conf->informationsrelease->dir_output . "/" . dol_sanitizeFileName($object->ref) . "/" . dol_sanitizeFileName($object->ref).".pdf");
    ...
    if ($action == "confirm_save_from_refuse" && GETPOST("confirm", 'alpha') == "yes" && $id > 0 && $user->rights->informationsrelease->creer)
    {
    	$object = new InformationsRelease($db);
    	$object->fetch($id);
    	...
    	if ($result > 0)
    	{
			...
			$emailFrom = $expediteur->email;
   			...
   			if ($emailFrom && $emailTo)
   			{
    			...
    			$subject = $societeName." - ".$langs->transnoentities("InformationsReleaseWaitingForReApproval");

    			// CONTENT
    			$link = $urlwithroot.'/informationsrelease/card.php?id='.$object->id;
				$link = '<a href="'.$link.'">'.$link.'</a>';
				$dateRefusEx = explode(" ", $object->date_refuse);
    			$message = $langs->transnoentities("InformationsReleaseWaitingForReApprovalMessage", $dateRefusEx[0], $object->detail_refuse, $expediteur->getFullName($langs), $link);

   				// Rebuild pdf
				/*
				$object->setDocModel($user,"");
				$resultPDF = informationsrelease_pdf_create($db,$object,'',"",$langs);
   				...
   				if($resultPDF)
   				{
   					...
   					$filename=array(); $filedir=array(); $mimetype=array();
   					array_push($filename,dol_sanitizeFileName($object->ref).".pdf");
   					array_push($filedir,$conf->informationsrelease->dir_output . "/" . dol_sanitizeFileName($object->ref) . "/" . dol_sanitizeFileName($object->ref_number).".pdf");
   					array_push($mimetype,"application/pdf");
				}
				*/
    ...
    // Approve
    if ($action == "confirm_approve" && GETPOST("confirm", 'alpha') == "yes" && $id > 0 && $user->rights->informationsrelease->approve)
    {
    	$object = new InformationsRelease($db);
    	$object->fetch($id);
    	...
    	if ($result > 0)
    	{
			...
			// FROM
   			...
   			if ($emailFrom && $emailTo)
   			{
    			...
    			$subject = $societeName." - ".$langs->transnoentities("InformationsReleaseApproved");

       			// CONTENT
       			$link = $urlwithroot.'/informationsrelease/card.php?id='.$object->id;
				$link = '<a href="'.$link.'">'.$link.'</a>';
       			$message = $langs->transnoentities("InformationsReleaseApprovedMessage", $object->ref, $destinataire->getFullName($langs), $expediteur->getFullName($langs), $link);
    ...
    if ($action == "confirm_refuse" && GETPOST('confirm', 'alpha') == "yes" && $id > 0 && $user->rights->informationsrelease->approve)
    {
    	$object = new InformationsRelease($db);
    	$object->fetch($id);
    	...
    	if ($result > 0)
    	{
    		...
    		if ($emailFrom && $emailTo)
    		{
    			...
    			$subject = $societeName." - ".$langs->transnoentities("InformationsReleaseRefused");

       			// CONTENT
       			$link = $urlwithroot.'/informationsrelease/card.php?id='.$object->id;
				$link = '<a href="'.$link.'">'.$link.'</a>';
    			$message = $langs->transnoentities("InformationsReleaseRefusedMessage", $object->ref, $destinataire->getFullName($langs), $expediteur->getFullName($langs), $detailRefuse, $link);
    ...
    if ($action == "confirm_cancel" && GETPOST('confirm', 'alpha') == "yes" && $id > 0 && $user->rights->informationsrelease->creer)
    {
	    	...
	    	if ($user->id == $object->fk_user_valid || $user->id == $object->fk_user_author)
	    	{
	    		...
	    		if ($result > 0)
	    		{
	    			...
	    			if ($emailFrom && $emailTo)
	    			{
	    			    ...
	    			    $subject = $societeName." - ".$langs->transnoentities("InformationsReleaseCanceled");

	    				// CONTENT
	    				$link = $urlwithroot.'/informationsrelease/card.php?id='.$object->id;
						$link = '<a href="'.$link.'">'.$link.'</a>';
	    				$message = $langs->transnoentities("InformationsReleaseCanceledMessage", $object->ref, $destinataire->getFullName($langs), $expediteur->getFullName($langs), $detailCancel, $link);
    ...
    if ($action == "confirm_setdraft" && GETPOST('confirm', 'alpha') == "yes" && $id > 0 && $user->rights->informationsrelease->creer)
    {
    	$object = new InformationsRelease($db);
    	$object->fetch($id);
    ...
    if ($action == 'set_unpaid' && $id > 0 && $user->rights->informationsrelease->to_paid)
    {
    	$object = new InformationsRelease($db);
    	$object->fetch($id);
    ...
    if ($action == 'set_paid' && $id > 0 && $user->rights->informationsrelease->to_paid)
    {
    	$object = new InformationsRelease($db);
    	$object->fetch($id);
    	...
    	if ($result > 0)
    	{
    		...
    		if ($emailFrom && $emailTo)
    		{
    			...
    			$subject = $societeName." - ".$langs->transnoentities("InformationsReleasePaid");

    			// CONTENT
    			$link = $urlwithroot.'/informationsrelease/card.php?id='.$object->id;
				$link = '<a href="'.$link.'">'.$link.'</a>';
    			$message = $langs->transnoentities("InformationsReleasePaidMessage", $object->ref, $destinataire->getFullName($langs), $expediteur->getFullName($langs), $link);
    ...
    if ($action == "addline" && $user->rights->informationsrelease->creer)
    {
    	$error = 0;
    	...
    	if (GETPOSTISSET('attachfile'))
    	{
    	    ...
    	    if (is_array($arrayoffiles) && !empty($arrayoffiles[0]))
    	    {
    	        ...
    	        $relativepath = 'informationsrelease/'.$object->ref.'/'.$arrayoffiles[0];
    	...
    	{
    		$langs->load("errors");
    		setEventMessages($langs->trans("WarningDateOfLineMustBeInInformationsReleaseRange"), null, 'warnings');
    	}
    ...
    if ($action == 'confirm_delete_line' && GETPOST("confirm", 'alpha') == "yes" && $user->rights->informationsrelease->creer)
    {
    	$object = new InformationsRelease($db);
    	$object->fetch($id);

    	$object_ligne = new InformationsReleaseLine($db);
    ...
    if ($action == "updateline" && $user->rights->informationsrelease->creer)
    {
    	$object = new InformationsRelease($db);
    	$object->fetch($id);
    	...
    	if (!$error)
    	{
    	    // TODO Use update method of InformationsReleaseLine
    ...
    $triggersendname = 'INFORMATIONSRELASE_SENTBYMAIL';
    $autocopy = 'MAIN_MAIL_AUTOCOPY_INFORMATIONSRELASE_TO';
    $trackid = 'exp'.$object->id;
    include DOL_DOCUMENT_ROOT.'/core/actions_sendmails.inc.php';

    // Actions to build doc
    $upload_dir = $conf->informationsrelease->dir_output;
    $permissiontoadd = $user->rights->informationsrelease->creer;
...
$title = $langs->trans("InformationsRelease")." - ".$langs->trans("Card");
...
//$paymentinformationsreleasestatic = new PaymentInformationsRelease($db);
...
if ($action == 'create')
{
    ...
    if (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->informationsrelease->writeall_advance)) $include_users = array();
	...
	print '<td>';
	$object = new InformationsRelease($db);
	$include_users = $object->fetch_users_approver_informationsrelease();
	if (empty($include_users)) print img_warning().' '.$langs->trans("NobodyHasPermissionToValidateInformationsRelease");
	else
	{
    	...
    	if (!empty($conf->global->INFORMATIONSRELASE_DEFAULT_VALIDATOR)) $defaultselectuser = $conf->global->INFORMATIONSRELASE_DEFAULT_VALIDATOR; // Can force default approver
    	if (GETPOST('fk_user_validator', 'int') > 0) $defaultselectuser = GETPOST('fk_user_validator', 'int');
    	$s = $form->select_dolusers($defaultselectuser, "fk_user_validator", 1, "", ((empty($defaultselectuser) || empty($conf->global->INFORMATIONSRELASE_DEFAULT_VALIDATOR_UNCHANGEABLE)) ? 0 : 1), $include_users);
	...
	// Payment mode
	if (!empty($conf->global->INFORMATIONSRELASE_ASK_PAYMENTMODE_ON_CREATION))
	{
		print '<tr>';
...
else
{
	if ($id > 0 || $ref)
	{
		...
		if ($result > 0)
		{
			...
			{
				if (empty($user->rights->informationsrelease->readall) && empty($user->rights->informationsrelease->lire_tous)
				    && (empty($conf->global->MAIN_USE_ADVANCED_PERMS) || empty($user->rights->informationsrelease->writeall_advance)))
				{
			...
			$head = informationsrelease_prepare_head($object);
			...
			if ($action == 'edit' && ($object->fk_statut < 3 || $object->fk_statut == 99))
			{
				...
				dol_fiche_head($head, 'card', $langs->trans("InformationsRelease"), 0, 'trip');
				...
				$linkback = '<a href="'.DOL_URL_ROOT.'/informationsrelease/list.php?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
			...
			{
				dol_fiche_head($head, 'card', $langs->trans("InformationsRelease"), -1, 'trip');

				// Clone confirmation
				if ($action == 'clone') {
				    // Create an array for form
				    $criteriaforfilter = 'hierarchyme';
				    if (!empty($user->rights->informationsrelease->readall)) $criteriaforfilter = '';
				    $formquestion = array(
				        'text' => '',
				    ...
				    $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('ToClone'), $langs->trans('ConfirmCloneInformationsRelease', $object->ref), 'confirm_clone', $formquestion, 'yes', 1);
				}
				...
				if ($object->fk_statut <= InformationsRelease::STATUS_VALIDATED)	// informed
				{
					print '<tr>';
				...
				elseif ($object->fk_statut == InformationsRelease::STATUS_CANCELED)
				{
					print '<tr>';
				...
				$rowspan = 5;
				if ($object->fk_statut <= InformationsRelease::STATUS_VALIDATED) $rowspan++;
				elseif ($object->fk_statut == InformationsRelease::STATUS_CANCELED) $rowspan += 2;
				else $rowspan += 2;
				if ($object->fk_statut == InformationsRelease::STATUS_REFUSED || !empty($object->detail_refuse)) $rowspan += 2;
				if ($object->fk_statut == InformationsRelease::STATUS_CLOSED) $rowspan += 2;
				print "</td>";
				print '</tr>';
				...
				// Payments already done (from payment on this informationsrelease)
				...
				$sql .= " FROM ".MAIN_DB_PREFIX."informationsrelease as e, ".MAIN_DB_PREFIX."payment_informationsrelease as p";
				$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."c_paiement as c ON p.fk_typepayment = c.id";
				$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'bank as b ON p.fk_bank = b.rowid';
				$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'bank_account as ba ON b.fk_account = ba.rowid';
				$sql .= " WHERE e.rowid = '".$id."'";
				$sql .= " AND p.fk_informationsrelease = e.rowid";
				$sql .= ' AND e.entity IN ('.getEntity('informationsrelease').')';
				$sql .= " ORDER BY dp";
				...
				if ($resql)
				{
				    ...
				    while ($i < $num)
				    {
				        ...
				        $paymentinformationsreleasestatic->id = $objp->rowid;
				        $paymentinformationsreleasestatic->datepaye = $db->jdate($objp->dp);
				        $paymentinformationsreleasestatic->ref = $objp->rowid;
				        $paymentinformationsreleasestatic->num_paiement = $objp->num_payment;
				        $paymentinformationsreleasestatic->num_payment = $objp->num_payment;
				        $paymentinformationsreleasestatic->payment_code = $objp->payment_code;

				        print '<tr class="oddseven">';
				        print '<td>';
						print $paymentinformationsreleasestatic->getNomUrl(1);
						print '</td>';
				    ...
				    if ($object->status == InformationsRelease::STATUS_REFUSED)
				    {
				...
				print '<form name="informationsrelease" action="'.$_SERVER["PHP_SELF"].'" enctype="multipart/form-data" method="post" >';
				print '<input type="hidden" name="token" value="'.newToken().'">';
				print '<input type="hidden" name="action" value="'.$actiontouse.'">';
				print '<input type="hidden" name="id" value="'.$object->id.'">';
				print '<input type="hidden" name="fk_informationsrelease" value="'.$object->id.'" />';
				...
				if (!empty($object->lines))
				{
					...
					if (($object->fk_statut < 2 || $object->fk_statut == 99) && $user->rights->informationsrelease->creer)
					{
					...
					foreach ($object->lines as &$line)
					{
						...
						if ($action != 'editline' || $line->rowid != GETPOST('rowid', 'int'))
						{
							...
							{
							    $modulepart = 'informationsrelease';
							    $maxheightmini = 32;

                                $result = $ecmfilesstatic->fetch($line->fk_ecm_files);
                                if ($result > 0)
                                {
                                    $relativepath = preg_replace('/informationsrelease\//', '', $ecmfilesstatic->filepath);
                                    ...
                                    if (image_format_supported($fileinfo['basename']) > 0)
                                    {
                                        ...
                                        if (!dol_is_file($conf->informationsrelease->dir_output.'/'.$relativepath.'/'.$minifile)) $minifile = getImageFileNameForSize($fileinfo['basename'], '_mini', '.png'); // For backward compatibility of old thumbs that were created with filename in lower case and with .png extension
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            $filepdf = $conf->informationsrelease->dir_output.'/'.$relativepath.'/'.$ecmfilesstatic->filename;
                                            $fileimage = $conf->informationsrelease->dir_output.'/'.$relativepath.'/'.$ecmfilesstatic->filename.'_preview.png';
							...
							// Ajout des boutons de modification/suppression
							if (($object->fk_statut < InformationsRelease::STATUS_VALIDATED || $object->fk_statut == InformationsRelease::STATUS_REFUSED) && $user->rights->informationsrelease->creer)
							{
						...
						if ($action == 'editline' && $line->rowid == GETPOST('rowid', 'int'))
						{
						    ...
						    print '</a>';
						    if (empty($conf->global->INFORMATIONSRELASE_DISABLE_ATTACHMENT_ON_LINES))
						    {
						    ...
						    $tredited = 'tredited';
						    include DOL_DOCUMENT_ROOT.'/informationsrelease/tpl/informationsrelease_addfile.tpl.php';
						    include DOL_DOCUMENT_ROOT.'/informationsrelease/tpl/informationsrelease_linktofile.tpl.php';
				...
				// Add a line
				if (($object->fk_statut == InformationsRelease::STATUS_DRAFT || $object->fk_statut == InformationsRelease::STATUS_REFUSED)
				    && $action != 'editline'
				    && $user->rights->informationsrelease->creer)
				{
				    $colspan = 11;
				    if (!empty($conf->global->MAIN_USE_EXPENSE_IK)) $colspan++;
				    if (!empty($conf->projet->enabled)) $colspan++;
					...
					print '<td class="right">';
					$defaultvat = -1;
					if (!empty($conf->global->INFORMATIONSRELASE_NO_DEFAULT_VAT)) $conf->global->MAIN_VAT_DEFAULT_IF_AUTODETECT_FAILS = 'none';
...
if ($action != 'create' && $action != 'edit')
{
	...
	// Send
	if (empty($user->socid)) {
		if ($object->fk_statut > InformationsRelease::STATUS_DRAFT) {
			//if ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) || $user->rights->informationsrelease->informationsrelease_advance->send)) {
	...
	*/
	if ($user->rights->informationsrelease->creer && $object->fk_statut == InformationsRelease::STATUS_DRAFT)
	{
		if (in_array($object->fk_user_author, $user->getAllChildIds(1)) || !empty($user->rights->informationsrelease->writeall_advance))
		{
			// Modify
	 ...
	 * 	Afficher : "Enregistrer" / "Modifier" / "Supprimer"
	 */
	if ($user->rights->informationsrelease->creer && $object->fk_statut == InformationsRelease::STATUS_REFUSED)
	{
		if ($user->id == $object->fk_user_author || $user->id == $object->fk_user_valid)
		{
			// Modify
	...
	if ($user->rights->informationsrelease->to_paid && $object->fk_statut == InformationsRelease::STATUS_APPROVED)
	{
		if ($user->id == $object->fk_user_author || $user->id == $object->fk_user_valid)
		{
			// setdraft
	 ...
	 */
	if ($object->fk_statut == InformationsRelease::STATUS_VALIDATED)
	{
		if (in_array($object->fk_user_author, $user->getAllChildIds(1)))
		{
			// set draft
	...
	if ($user->rights->informationsrelease->approve && $object->fk_statut == InformationsRelease::STATUS_VALIDATED)
	{
	...
	if ($user->rights->informationsrelease->approve && $object->fk_statut == InformationsRelease::STATUS_APPROVED)
	{
	    print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?action=refuse&id='.$object->id.'">'.$langs->trans('Deny').'</a></div>';
	}

	// If bank module is used
	if ($user->rights->informationsrelease->to_paid && !empty($conf->banque->enabled) && $object->fk_statut == InformationsRelease::STATUS_APPROVED)
	{
		// Pay
		if ($remaintopay == 0)
		{
		...
		{
			print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/informationsrelease/payment/payment.php?id='.$object->id.'&amp;action=create">'.$langs->trans('DoPayment').'</a></div>';
		}
	}
	...
	if ($user->rights->informationsrelease->creer && ($user->id == $object->fk_user_author || $user->id == $object->fk_user_valid) && $object->fk_statut == InformationsRelease::STATUS_APPROVED)
	{
    	// Cancel
   		print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?action=cancel&id='.$object->id.'">'.$langs->trans('Cancel').'</a></div>';
	}
	...
	if (($user->rights->informationsrelease->approve || $user->rights->informationsrelease->to_paid) && $object->fk_statut == InformationsRelease::STATUS_CLOSED)
	{
	    // Cancel
	    print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?action=cancel&id='.$object->id.'">'.$langs->trans('Cancel').'</a></div>';
	}

	if ($user->rights->informationsrelease->to_paid && $object->paid && $object->fk_statut == InformationsRelease::STATUS_CLOSED)
	{
		// Set unpaid
	...
	// Clone
	if ($user->rights->informationsrelease->creer) {
	...
	if ($user->rights->informationsrelease->creer && $user->id == $object->fk_user_author && $object->fk_statut < InformationsRelease::STATUS_APPROVED)
	{
	    // Delete
		print '<div class="inline-block divButAction"><a class="butActionDelete" href="'.$_SERVER["PHP_SELF"].'?action=delete&id='.$object->id.'">'.$langs->trans('Delete').'</a></div>';
	}
	elseif ($user->rights->informationsrelease->supprimer && $object->fk_statut != InformationsRelease::STATUS_CLOSED)
	{
    	// Delete
	    print '<div class="inline-block divButAction"><a class="butActionDelete" href="'.$_SERVER["PHP_SELF"].'?action=delete&id='.$object->id.'">'.$langs->trans('Delete').'</a></div>';
	}
...
// FICHIERS JOINTS
	if ($user->rights->informationsrelease->creer && $action != 'create' && $action != 'edit')
	{
		$filename = dol_sanitizeFileName($object->ref);
		$filedir = $conf->informationsrelease->dir_output."/".dol_sanitizeFileName($object->ref);
		$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;
		...
		$delallowed	= $user->rights->informationsrelease->creer;
		$var = true;
		print $formfile->showdocuments('informationsrelease', $filename, $filedir, $urlsource, $genallowed, $delallowed);
		$somethingshown = $formfile->numoffiles;
	}

	// Disabled for informationsrelease, there is no thirdparty on informationsrelease, so nothing to define the list of other object we can suggest to link to
	/*
	if ($action != 'create' && $action != 'edit' && ($id || $ref))
	{
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('informationsrelease'));
    ...
    */
	...
	$somethingshown = $formactions->showactions($object, 'informationsrelease', null);

	print '</div></div></div>';
}

// Presend form
$modelmail = 'informationsrelease';
$defaulttopic = 'SendInformationsReleaseRef';
$diroutput = $conf->informationsrelease->dir_output;
$trackid = 'exp'.$object->id;
