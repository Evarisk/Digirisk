...
dol_include_once('/digiriskdolibarr/core/modules/digiriskdolibarr/mod_firepermit_jupiter.php');
dol_include_once('/digiriskdolibarr/core/modules/digiriskdolibarr/mod_firepermit_standard.php');
dol_include_once('/digiriskdolibarr/core/modules/digiriskdolibarr/mod_firepermit_advanced.php');
dol_include_once('/digiriskdolibarr/core/modules/digiriskdolibarr/mod_firepermit_encelade.php');
...
$permissiontodelete = $user->rights->digiriskdolibarr->firepermit->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
$permissionnote = $user->rights->digiriskdolibarr->firepermit->write;
$permissiondellink = $user->rights->digiriskdolibarr->firepermit->write;
$upload_dir = $conf->digiriskdolibarr->multidir_output[isset($object->entity) ? $object->entity : 1];
...
if (empty($reshook))
{
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			...
			else $backtopage = dol_buildpath('/digiriskdolibarr/firepermit_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
		}
	}
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewFirePermit"), '', 'object_'.$object->picto);
	...
	$digirisk_addon = $conf->global->DIGIRISKDOLIBARR_FIREPERMIT_ADDON;
...
{
	print load_fiche_titre($langs->trans("FirePermit"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	dol_fiche_head($head, 'card', $langs->trans("FirePermit"), -1, $object->picto);
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/digiriskdolibarr/firepermit_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	if ($action != 'presend')
	{
		...
		if ($includedocgeneration) {
			...
			$genallowed = $user->rights->digiriskdolibarr->firepermit->read;	// If you can read, you can build the PDF to read content
			$delallowed = $user->rights->digiriskdolibarr->firepermit->create;	// If you can create/edit, you can remove a file on card

			print $formfile->showdocuments('digiriskdolibarr:FirePermit',$dir_files, $filedir, $urlsource, $genallowed, $delallowed, $conf->global->DIGIRISKDOLIBARR_FIREPERMIT_DEFAULT_MODEL, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);

		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('firepermit'));
	...
	$object->modelpdf = 'firepermit_A4_odt';

	//get title and content
	$modelmail = 'Digirisk_FirePermit';
	$defaulttopic = $langs->trans('SendFirePermit') . ' - ' . $conf->global->MAIN_INFO_SOCIETE_NOM;
	$diroutput = $conf->digiriskdolibarr->dir_output . '/firepermit';
	$trackid = 'firepermit'.$object->id;
