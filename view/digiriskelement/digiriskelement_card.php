...
if (($id || $ref) && $action == 'edit') {
	...
	if ($id != $conf->global->DIGIRISKDOLIBARR_DIGIRISKELEMENT_TRASH) {
		...
		print $object->selectDigiriskElementList($object->fk_parent, 'fk_parent', ['customsql' => 't.element_type = \'groupment\' AND t.rowid NOT IN (' . rtrim(implode(',', $deletedElements) . ',' . implode(',', $childrenElements), ',') . ')'], 0, 0, [], 0, 0, 'minwidth100 maxwidth300', GETPOST('id'));
	}
