...
if ((empty($action) || ($action != 'create' && $action != 'edit'))) {
	...
	if ($object->id > 0) {
		...
		if (empty($reshook) && $permissiontoadd) {
			...
			if ($filesigned == 0) {
				$modellist = getListOfModelsDigirisk($db, 'preventionplandocument');
				if (!empty($modellist)) {
					asort($modellist);
			...
			print '<a class="' . ($object->status == 3 ? 'butAction' : 'butActionRefused classfortooltip') . '" id="actionButtonArchive" title="' . ($object->status == 3 ? '' : dol_escape_htmltag($langs->trans("PreventionPlanMustBeLocked"))) . '" href="' . ($object->status == 3 ? ($_SERVER["PHP_SELF"] . '?id=' . $object->id . '&action=setArchived') : '#') . '">' . $langs->trans("Archive") . '</a>';
			...
			$permissiontodelete ? print '<a class="' . ($object->status != 0 ? 'butActionDelete' : 'butActionRefused classfortooltip') . '" id="actionButtonDelete" title="' . ($object->status != 0 ? '' : dol_escape_htmltag($langs->trans("PreventionPlanMustBeInProgress"))) . '" href="' . ($object->status != 0 ? ($_SERVER["PHP_SELF"] . '?id=' . $object->id . '&action=delete') : '#') . '">' . $langs->trans("Delete") . '</a>' : '';
