...
$hookmanager->initHooks(['preventionplanlist']); // Note that conf->hooks_modules contains array.
...
foreach ($object->fields as $key => $val) {
	...
	if (preg_match('/^(date|timestamp|datetime)/', $val['type'])) {
		$search[$key . '_date_start'] = dol_mktime(0, 0, 0, GETPOST('search_' . $key . '_date_startmonth', 'int'), GETPOST('search_' . $key . '_date_startday', 'int'), GETPOST('search_' . $key . '_date_startyear', 'int'));
		$search[$key . '_date_end'] = dol_mktime(23, 59, 59, GETPOST('search_' . $key . '_date_endmonth', 'int'), GETPOST('search_' . $key . '_date_endday', 'int'), GETPOST('search_' . $key . '_date_endyear', 'int'));
	}
}
...
if (empty($reshook)) {
	...
	if (GETPOST('button_removefilter_x', 'alpha') || GETPOST('button_removefilter.x', 'alpha') || GETPOST('button_removefilter', 'alpha')) { // All tests are required to be compatible with all browsers.
		foreach ($object->fields as $key => $val) {
			$search[$key] = '';
			if (preg_match('/^(date|timestamp|datetime)/', $val['type'])) {
				$search[$key.'_date_start'] = '';
				$search[$key.'_date_end'] = '';
			}
		}
		$toselect              = [];
